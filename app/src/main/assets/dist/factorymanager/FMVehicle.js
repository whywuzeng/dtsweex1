...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/factorymanager/FMVehicle.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-3d22c3db"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
    name: 'FMVehicle',
    components: { WxcMinibar: _weexUi.WxcMinibar, WxcCell: _weexUi.WxcCell, WxcDialog: _weexUi.WxcDialog, WxcLoading: _weexUi.WxcLoading },
    data: function data() {
        return {
            list: [],
            listNum: '', //选择的第几个
    ...
    methods: {
        ...
        rightButton: function rightButton() {
            var num = this.$getConfig().bundleUrl.indexOf("FMVehicle.js");
            var bundleUrl = this.$getConfig().bundleUrl.substr(0, num);

            navigator.push({
                url: bundleUrl + "FMMessage.js",
                animated: "true"
            }, function (event) {});
        },
        ...
        // 叫号保存
        saveCall: function saveCall() {
            // 备注 缺货等待 时提示
            if (this.itemList.Remarks == '缺货等待') {
                this.markshow = true;
                return;
            }
            // 判断是否答合
            if (this.itemList.IsFlag == 'N') {
                this.Noshow = true;
                return;
            }
            // 除了第1个，其他的都需要填写叫号原因
            if (this.listNum != 0) {
                ...
                // 中文转换
                var sSpaceName = encodeURIComponent(this.SpaceName);
                var sTruckNo = encodeURIComponent(this.itemList.TruckNo);

                stream.fetch({
                    method: 'POST',
                    url: app_server_url + 'Stack_Scheduling/ManualCall?sQueueId=' + this.parentItem.QueueID + '&sSpaceID=' + this.sSpaceID + '&sSpaceName=' + sSpaceName + '&sBill_Id=' + this.itemList.Bill_Id + '&sTruckNo=' + sTruckNo + '&sResonId=null&sResonName=null&sMemo=null&sUserId=' + this.sUserID + '&sInType=' + this.parentItem.sInType,
                    type: 'json'
                }, function (ret) {
                    if (ret.statusText == 'OK') {
                        if (ret.data.Data.ManualCallReturn[0].ai_value == -1) {
                            modal.toast({ message: ret.data.Data.ManualCallReturn[0].as_msg });
                        } else {
                            modal.toast({ message: '叫号成功' });
                            storage.setItem('backRefresh', 'true');
        ...
        dialogSelection: function dialogSelection(item) {
            ...
            item.checked = true;
            // 保存选中id/名称
            this.sResonId = item.id;
            this.sResonName = item.text;
        },
        ...
        // 不符合确定
        ConfirmBtnClicked: function ConfirmBtnClicked() {
            // 隐藏不合号
            this.Noshow = false;
        ...
        wxcDialogConfirmBtnClicked: function wxcDialogConfirmBtnClicked() {
            ...
            // 当选择其他的时候为必填
            if (this.sResonId == '107004') {
                if (this.sMemo == '') {
                    modal.toast({ message: '选择其他时，备注为必填项' });
                    return;
                }
            }
            ...
            var sSpaceName = encodeURIComponent(this.SpaceName);
            var sTruckNo = encodeURIComponent(this.itemList.TruckNo);
            var sResonName = encodeURIComponent(this.sResonName);
            var sMemo = null;
            if (this.sMemo == '') {
                sMemo = encodeURIComponent("无");
            } else {
                sMemo = encodeURIComponent(this.sMemo);
            }
            ...
            if (this.jhType == 1) {
                ...
                stream.fetch({
                    method: 'POST',
                    url: app_server_url + 'Stack_Scheduling/ManualCall?sQueueId=' + this.parentItem.QueueID + '&sSpaceID=' + this.sSpaceID + '&sSpaceName=' + sSpaceName + '&sBill_Id=' + this.itemList.Bill_Id + '&sTruckNo=' + sTruckNo + '&sResonId=' + this.sResonId + '&sResonName=' + sResonName + '&sMemo=' + sMemo + '&sUserId=' + this.sUserID + '&sInType=' + this.parentItem.sInType,
                    type: 'json'
                }, function (ret) {
                    if (ret.statusText == 'OK') {
            ...
            } else if (this.jhType == 2) {
                // 插队确定保存
                stream.fetch({
                    method: 'POST',
                    url: app_server_url + 'Stack_Scheduling/InsertQueueTruck?sSpaceID=' + this.sSpaceID + '&sBill_Id=' + this.Bill_Id + '&sResonId=' + this.sResonId + '&sResonName=' + sResonName + '&sMemo=' + sMemo + '&sUserId=' + this.sUserID,
                    type: 'json'
                }, function (ret) {
                    if (ret.statusText == 'OK') {
                        modal.toast({ message: '插队成功' });
                        storage.setItem('backRefresh', 'true');
                        navigator.pop({ animated: "true" }, function (event) {});
                    } else {
                        modal.toast({ message: '网络访问失败' });
    ...
    created: function created() {
        ...
        storage.getItem("sUserID", function (event) {
            _this.sUserID = event.data;
        });
        // 判断类型 叫号1、插队2
        storage.getItem("jhType", function (event) {
            _this.jhType = event.data;
        });

        // 上一条垛口信息
        storage.getItem("itemList", function (event) {
            var data = JSON.parse(event.data);

            // sInType=2缓冲区 sInType=1垛口区
            if (data.sInType == 2) {
                _this.SpaceName = data.BufferName;
                _this.sSpaceID = data.BufferID;
                _this.parentItem = data;
                // 获取车辆列表接口
                stream.fetch({
                    method: 'GET',
                    url: app_server_url + 'Stack_Scheduling/GetQueueTruckList?sQueueId=' + data.QueueID + '&sSpaceID=' + data.BufferID + '&sInType=' + data.sInType,
                    type: 'json'
                }, function (ret) {
                    //关闭loading
                    _this.isShow = false;
                    if (ret.statusText == 'OK') {
                        ret.data.Data.ParkingSpaceList.forEach(function (val) {
                            val.checked = false;
                        });
                        _this.list = ret.data.Data.ParkingSpaceList;
                    } else {
                        modal.toast({ message: '网络访问失败' });
                    }
                });
                ...
                if (_this.isipx) {
                    _this.scrollHeight = bodyHeight - 533 - 66;
                } else if (_this.isiphone) {
                    _this.scrollHeight = bodyHeight - 533 - 40;
                } else {
                    _this.scrollHeight = bodyHeight - 533 - 40;
                }
            } else if (data.sInType == 1) {
                _this.SpaceName = data.SpaceName;
                _this.sSpaceID = data.SpaceID;
                _this.parentItem = data;
                if (_this.parentItem.SpaceStatus == 0) {
                    _this.parentItem.SpaceStatus = '指派';
                } else if (_this.parentItem.SpaceStatus == 1) {
                    _this.parentItem.SpaceStatus = '空闲';
                } else if (_this.parentItem.SpaceStatus == 2) {
                    _this.parentItem.SpaceStatus = '业务操作';
                }
                // 获取车辆列表接口
                stream.fetch({
                    method: 'GET',
                    url: app_server_url + 'Stack_Scheduling/GetQueueTruckList?sQueueId=' + data.QueueID + '&sSpaceID=' + data.SpaceID + '&sInType=' + data.sInType,
                    type: 'json'
                }, function (ret) {
                    //关闭loading
                    _this.isShow = false;
                ...
                // 列表高度
                var _bodyHeight = _weexUi.Utils.env.getScreenHeight();
                if (_this.isipx) {
                    _this.scrollHeight = _bodyHeight - 615 - 66;
                } else if (_this.isiphone) {
                    _this.scrollHeight = _bodyHeight - 615 - 40;
                } else {
                    _this.scrollHeight = _bodyHeight - 615 - 40;
                }
            }
        ...
        // 获取原因下拉 type=107固定值
        setTimeout(function () {
            //modal.toast({message:`${app_server_url}Stack_Scheduling/GetBaseDataList?Type=107`});
            stream.fetch({
                method: 'GET',
                url: app_server_url + 'Stack_Scheduling/GetBaseDataList?Type=107',
                type: 'json'
            }, function (ret) {
                if (ret.statusText == 'OK') {
                    ret.data.Data.BaseDataList.forEach(function (val) {
                        val.checked = false;
                    });
                    // 绑定值
                    ret.data.Data.BaseDataList[0].checked = true;
                    _this.sResonId = ret.data.Data.BaseDataList[0].id;
                    _this.sResonName = ret.data.Data.BaseDataList[0].text;
                    // 绑定列表
                    _this.dialogList = ret.data.Data.BaseDataList;
                } else {
                    modal.toast({ message: '网络访问失败' });
                }
            });
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    slot: "right"
  }, [_vm._v("")])]), (_vm.parentItem.sInType == 1) ? _c('div', {
    staticClass: ["baseTop"]
  }, [_c('div', {
    staticClass: ["baseTop-box", "baseTop-box-border"]
  }, [_c('text', {
    staticClass: ["baseTop-text", "orange"]
  }, [_vm._v("垛口ID：" + _vm._s(_vm.parentItem.SpaceID))]), _c('text', {
    staticClass: ["baseTop-text", "orange"]
  }, [_vm._v("当前状态：" + _vm._s(_vm.parentItem.SpaceStatus))])]), _c('div', {
    staticClass: ["baseTop-box"]
  }, [_c('text', {
    staticClass: ["baseTop-text"]
  }, [_vm._v("运输类型")]), _c('text', {
    staticClass: ["baseTop-text"]
  }, [_vm._v(_vm._s(_vm.parentItem.TransportTypeName))])]), _c('div', {
    staticClass: ["baseTop-box"]
  }, [_c('text', {
    staticClass: ["baseTop-text"]
  }, [_vm._v("装卸类型")]), _c('text', {
    staticClass: ["baseTop-text"]
  }, [_vm._v(_vm._s(_vm.parentItem.SpaceType))])]), _c('div', {
    staticClass: ["baseTop-box"]
  }, [_c('text', {
    staticClass: ["baseTop-text"]
  }, [_vm._v("可进车型")]), _c('text', {
    staticClass: ["baseTop-text"]
  }, [_vm._v(_vm._s(_vm.parentItem.TruckTypeList))])]), _c('div', {
    staticClass: ["baseTop-box"]
  }, [_c('text', {
    staticClass: ["baseTop-text"]
  }, [_vm._v("可进车长")]), _c('text', {
    staticClass: ["baseTop-text"]
  }, [_vm._v(_vm._s(_vm.parentItem.TruckLengthList))])])]) : (_vm.parentItem.sInType == 2) ? _c('div', {
    staticClass: ["baseTop", "jcbaseTop"]
  }, [_c('div', {
    staticClass: ["baseTop-box"]
  }, [_c('text', {
    staticClass: ["baseTop-text"]
  }, [_vm._v("缓冲区编号")]), _c('text', {
    staticClass: ["baseTop-text"]
  }, [_vm._v(_vm._s(_vm.parentItem.BufferID))])]), _c('div', {
    staticClass: ["baseTop-box"]
  }, [_c('text', {
    staticClass: ["baseTop-text"]
  }, [_vm._v("名称")]), _c('text', {
    staticClass: ["baseTop-text"]
  }, [_vm._v(_vm._s(_vm.parentItem.BufferName))])]), _c('div', {
    staticClass: ["baseTop-box"]
  }, [_c('text', {
    staticClass: ["baseTop-text"]
  }, [_vm._v("最大停车数")]), _c('text', {
    staticClass: ["baseTop-text"]
  }, [_vm._v(_vm._s(_vm.parentItem.Number))])]), _c('div', {
    staticClass: ["baseTop-box"]
  }, [_c('text', {
    staticClass: ["baseTop-text"]
  }, [_vm._v("当前停车数")]), _c('text', {
    staticClass: ["baseTop-text"]
  }, [_vm._v(_vm._s(_vm.parentItem.UseNumber))])])]) : _vm._e(), _vm._m(0), _c('wxc-loading', {
    attrs: {
      "show": _vm.isShow
    }
  }), _c('scroller', {
    ...
    return _c('div', {
      ...
      on: {
        "click": function($event) {
          _vm.selection(item, index)
        }
      }
    }, [(item.checked) ? _c('text', {
      staticClass: ["iconfont", "veh-header-icon", "red"]
    }, [_vm._v("")]) : _c('text', {
      staticClass: ["iconfont", "veh-header-icon"]
    }, [_vm._v("")]), _c('text', {
      staticClass: ["veh-header-text"]
    }, [_vm._v(_vm._s(item.SerialNumber))]), _c('text', {
      staticClass: ["veh-header-text", "font22"]
    }, [_vm._v(_vm._s(item.TruckNo))]), _c('text', {
      staticClass: ["veh-header-text"]
    }, [_vm._v(_vm._s(item.IsSource))]), _c('text', {
      staticClass: ["veh-header-text"]
    }, [_vm._v(_vm._s(item.WaitTime))]), _c('text', {
      staticClass: ["veh-header-text"]
    }, [_vm._v(_vm._s(item.Mark))]), _c('text', {
      staticClass: ["veh-header-text"]
    }, [_vm._v(_vm._s(item.Remarks))]), _c('text', {
      staticClass: ["veh-header-text"]
    }, [_vm._v(_vm._s(item.CommodityCode))]), _c('text', {
      staticClass: ["veh-header-text", "blue"],
      on: {
        "click": function($event) {
          _vm.billId(item)
        }
  ...
  }, [_vm._v("目前没有车辆可选")]) : _vm._e()], 2), _c('div', {
    staticClass: ["veh-bottom"]
  }, [_c('div', {
    staticClass: ["veh-bottom-left"]
  }, [_c('text', {
    staticClass: ["veh-bottom-text1"]
  }, [_vm._v("请选择前往的车辆")]), (!_vm.isChecked) ? _c('text', {
    staticClass: ["veh-bottom-text2"]
  }, [_vm._v(_vm._s(_vm.TruckName))]) : _vm._e()]), (_vm.jhType == 1 && !_vm.isChecked) ? _c('div', {
    staticClass: ["veh-bottom-right"],
    on: {
      "click": _vm.saveCall
    }
  }, [_c('text', {
    staticClass: ["veh-bottom-rText"]
  }, [_vm._v("立即叫号")])]) : (_vm.jhType == 1 && _vm.isChecked) ? _c('div', {
    staticClass: ["veh-bottom-right", "graybg"]
  }, [_c('text', {
    staticClass: ["veh-bottom-rText"]
  }, [_vm._v("立即叫号")])]) : (_vm.jhType == 2 && !_vm.isChecked) ? _c('div', {
    staticClass: ["veh-bottom-right"],
    on: {
    ...
    staticClass: ["veh-bottom-rText"]
  }, [_vm._v("立即插队")])]) : (_vm.jhType == 2 && _vm.isChecked) ? _c('div', {
    staticClass: ["veh-bottom-right", "graybg"]
  }, [_c('text', {
    staticClass: ["veh-bottom-rText"]
  }, [_vm._v("立即插队")])]) : _vm._e()]), _c('wxc-dialog', {
    attrs: {
      "title": "系统提示",
      "show": _vm.markshow,
      "single": false,
    ...
    staticClass: ["font30"]
  }, [_vm._v("此车辆缺货等待中，请确认是否叫号?")])])]), _c('wxc-dialog', {
    attrs: {
      "title": "不符合条件",
      "show": _vm.Noshow,
      "single": false,
    ...
    on: {
      "wxcDialogCancelBtnClicked": _vm.CancelBtnClicked,
      "wxcDialogConfirmBtnClicked": _vm.ConfirmBtnClicked
    }
  }, [_c('div', {
    attrs: {
      "slot": "content"
    ...
    staticClass: ["font30"]
  }, [_vm._v("当前车辆与垛口不符合，是否继续?")])])]), _c('wxc-dialog', {
    attrs: {
      "title": "请选择原因",
      "show": _vm.show,
      "single": false,
    ...
    on: {
      "input": function($event) {
        _vm.sMemo = $event.target.attr.value
      }
    }
  })], 2)])], 1)
  ...
  return _c('div', {
    ...
    staticClass: ["veh-header-text", "veh-header-textB", "width140"]
  }, [_vm._v("车牌")]), _c('text', {
    staticClass: ["veh-header-text", "veh-header-textB"]
  }, [_vm._v("来源点")]), _c('text', {
    staticClass: ["veh-header-text", "veh-header-textB"]
