...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/factorymanager/FMStack.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-4cf3fa12"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
    ...
    methods: {
        ...
        minibarRightButtonClick: function minibarRightButtonClick() {
            var num = this.$getConfig().bundleUrl.indexOf("FMStack.js");
        ...
        saveModel: function saveModel() {
            ...
            // 中文转码
            var sSpaceName = encodeURIComponent(this.itemList.SpaceName);
            var sTruckNo = encodeURIComponent(this.parentItem.TruckID);
            stream.fetch({
                method: 'POST',
                url: app_server_url + 'Stack_Scheduling/SpaceChange?sSpaceID=' + this.itemList.SpaceID + '&sSpaceName=' + sSpaceName + '&sBill_Id=' + this.parentItem.Bill_Id + '&sTruckNo=' + sTruckNo,
                type: 'json'
            }, function (ret) {
                if (ret.statusText == 'OK') {
                    modal.toast({ message: '调整垛口成功' });

                    var num = _this.$getConfig().bundleUrl.indexOf("FMStack.js");
                    var bundleUrl = _this.$getConfig().bundleUrl.substr(0, num);
                    navigator.push({
                        url: bundleUrl + "FMTabs.js",
                        animated: "true"
                    }, function (event) {});
                } else {
    ...
    created: function created() {
        ...
        storage.getItem("itemList", function (event) {
            _this2.parentItem = JSON.parse(event.data);
        });

        storage.getItem("sQueueId", function (event) {
            // 获取车辆列表接口
            stream.fetch({
                method: 'GET',
                url: app_server_url + 'Stack_Scheduling/GetParkingSpaceSelectList?sQueueId=' + event.data,
                type: 'json'
            }, function (ret) {
                if (ret.statusText == 'OK') {
                    ret.data.Data.ParkingSpaceSelectList.forEach(function (val) {
                        val.checked = false;
                    });
                    _this2.list = ret.data.Data.ParkingSpaceSelectList;
                    //关闭loading
                    _this2.isShow = false;
                } else {
                    modal.toast({ message: '网络访问失败' });
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    slot: "right"
  }, [_vm._v("")])]), _vm._m(0), _c('wxc-loading', {
    attrs: {
      "show": _vm.isShow
    }
  }), _c('scroller', {
    ...
    }, [_vm._v(_vm._s(item.SpaceName))]), _c('text', {
      staticClass: ["veh-header-text"]
    }, [_vm._v(_vm._s(item.SpaceType))])])
  }), (_vm.list.length == 0) ? _c('text', {
    staticStyle: {
      textAlign: "center",
      marginTop: "50px"
    }
  }, [_vm._v("目前没有垛口可选")]) : _vm._e()], 2), _c('div', {
    staticClass: ["veh-bottom"]
  }, [_c('div', {
    staticClass: ["veh-bottom-left"]
  }, [_c('text', {
    staticClass: ["veh-bottom-text1"]
  }, [_vm._v("移到指定垛口")]), (!_vm.isChecked) ? _c('text', {
    staticClass: ["veh-bottom-text2"]
  }, [_vm._v(_vm._s(_vm.itemList.SpaceName) + "-" + _vm._s(_vm.itemList.SpaceType))]) : _vm._e()]), (!_vm.isChecked) ? _c('div', {
