...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/factorymanager/FMAssessDetail.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-480d1b36"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
module.exports = {
  ...
  "poptitle-text": {
    ...
    "borderLeftWidth": "1",
    "borderColor": "#dddddd",
    "borderBottomLeftRadius:first": "10",
    "borderTopLeftRadius:first": "10",
    "borderBottomRightRadius:last": "10",
    "borderTopRightRadius:last": "10"
  },
  "poptitle-text-active": {
    "backgroundColor": "#c00000",
    "color": "#FFFFFF"
  },
  "popdetail": {
...
exports.default = {
    name: "FMAssessDetail",
    components: { WxcButton: _weexUi.WxcButton, WxcMinibar: _weexUi.WxcMinibar, WxcLoading: _weexUi.WxcLoading, WxcPartLoading: _weexUi.WxcPartLoading, Utils: _weexUi.Utils, WxcPopup: _weexUi.WxcPopup, WxcCell: _weexUi.WxcCell },
    data: function data() {
        return {
            lodinisShow: false,
            lists1: [1, 1, 1, 1, 0],
    ...
    methods: {
        // 打分评价
        start1: function start1(item, index) {
            // 只读时不能修改
            if (this.parentItem.isEvaluate == 'Y') {
                return;
            }
            // 不能打5分
            if (index + 1 == 5) {
            ...
            if (this.numbre1 != index + 1) {
                this.putselect = [];
                this.popDetailAll.forEach(function (val) {
                    val.checked = true;
                });
            }
            this.numbre1 = index + 1;
            this.lists1 = [0, 0, 0, 0, 0];
        ...
        openBottomPopup: function openBottomPopup() {
            if (this.numbre1 == 5) return false;
        ...
        // 切换类别
        selectTop: function selectTop(item, index) {
            this.popIndex = item.Type;
        },

        // 类别详情选中
        selectDetail: function selectDetail(item, index) {
            // 为假时不能选中
            if (!item.checked) return false;
            // 添加到列表
            item.checked = false;
            this.putselect.push(item);
            this.isBottomShow = false;
        },

        // 获取类别
        getTsetContentType: function getTsetContentType() {
            var _this = this;

            stream.fetch({
                method: "GET",
                url: app_server_url + "Stack_Scheduling/GetTsetContentType?UserID=" + this.UserId,
                type: "json",
                timeout: 20000
            }, function (ret) {
                if (ret.ok === false) {
                    ...
                    _this.popTitle = ret.data.Data.result;
                }
            });
        },

        // 获取小的分类
        getTsetContentList: function getTsetContentList() {
            var _this2 = this;

            stream.fetch({
                method: "GET",
                url: app_server_url + "Stack_Scheduling/GetTsetContentList?UserID=" + this.UserId,
                type: "json",
                timeout: 20000
            }, function (ret) {
                ...
                if (ret.ok === false) {
                    ...
                    // 给数据添加属性
                    ret.data.Data.result.forEach(function (val) {
                        val.checked = true;
                    });
                    _this2.popDetailAll = ret.data.Data.result;
                }
            });
        },

        // 已评价状态
        getTsetContentedInfoQuery: function getTsetContentedInfoQuery() {
            var _this3 = this;

            stream.fetch({
                method: "GET",
                url: app_server_url + "Stack_Scheduling/GetTsetContentedInfoQuery?sBillId=" + this.parentItem.Bill_id + "&sQueueId=" + this.parentItem.QueueID,
                type: "json",
                timeout: 20000
            }, function (ret) {
                if (ret.ok === false) {
                    ...
                    // 星号初始化
                    _this3.lists1 = [0, 0, 0, 0, 0];
                    for (var i = 0; i <= ret.data.Data.result[0].Level - 1; i++) {
                        _this3.lists1[i] = 1;
                    }

                    _this3.remarks = ret.data.Data.result[0].Remarks;
                    // 只读数据
                    if (ret.data.Data.result[0].TsetShortContent != null) {
                        _this3.putselect = ret.data.Data.result;
                    }
                }
            });
        },
        ...
        // 删除违规项
        closeList: function closeList(item, index) {
            this.putselect.splice(index, 1);
            // 删除后更新状态
            this.popDetailAll.forEach(function (val) {
                if (val.testID == item.testID) {
                    val.checked = true;
                    val.FillMoney = null;
                }
            });
        ...
        // 提交评价
        saveCall: function saveCall(e) {
            var onOff = false;
            var TypeIds = []; //id
            var FillNums = []; //价格
            // 清空列表名称
            this.putselect.forEach(function (val) {
                if (val.FillMoney == null || val.FillMoney == '') {
                    onOff = true;
                }
                TypeIds.push(val.testID);
                FillNums.push(val.FillMoney);
            });

            if (onOff) {
                modal.toast({ message: '金额不能为空' });return;
            }

            var list = JSON.stringify(this.putselect);
            // 备注
            var remarks = '';
            if (this.remarks != '') {
            ...
            stream.fetch({
                method: "POST",
                url: app_server_url + "Stack_Scheduling/InsertCheckIndex?sBillId=" + this.parentItem.Bill_id + "&sQueueId=" + this.parentItem.QueueID + "&iLevel=" + this.numbre1 + "&sUserId=" + this.UserId + "&sRemarks=" + (remarks == '' ? null : remarks) + "&TypeIds=" + (TypeIds.toString() == '' ? null : TypeIds.toString()) + "&FillNums=" + (FillNums.toString() == '' ? null : FillNums.toString()),
                type: "json"
            }, function (ret) {
                ...
                if (ret.ok === false) {
                    ...
                    modal.toast({ message: "评价成功" });
                    storage.setItem('historyRefresh', 'true');
    ...
    created: function created() {
        ...
        storage.getItem(_app_constants.app_userid, function (event) {
            if (event.result == "success") {
                ...
                setTimeout(function () {
                    _this4.getTsetContentType();
                    // 调用子类列表
                    _this4.getTsetContentList();
                }, 100);
            }
        });
        ...
        storage.getItem("TMHistoryOrdersBill_id", function (event) {
            ...
            _this4.parentItem = event.data;

            // 调用已评价接口
            if (_this4.parentItem.isEvaluate == 'Y') {
                _this4.getTsetContentedInfoQuery();
            }
        });
    }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('div', {
    ...
    slot: "value"
  }, [_vm._v(_vm._s(_vm.parentItem.Bill_id))])]), _c('div', {
    staticClass: ["div-context"]
  }, [_c('div', {
    staticClass: ["div-xmain"]
  }, _vm._l((_vm.lists1), function(item, index) {
    return _c('div', {
      key: index,
      staticClass: ["div-x"],
      on: {
        "click": function($event) {
          _vm.start1(item, index)
        }
      }
    }, [(item == 1) ? _c('text', {
      staticClass: ["text-h", "iconfont", "red"]
    }, [_vm._v("")]) : _c('text', {
      staticClass: ["text-h", "iconfont"]
    }, [_vm._v("")])])
  })), _c('textarea', {
    staticClass: ["textarea"],
    attrs: {
      "placeholder": "填写备注",
      "disabled": _vm.parentItem.isEvaluate == 'Y',
      "value": (_vm.remarks)
    },
    on: {
      "input": function($event) {
        _vm.remarks = $event.target.attr.value
      }
    }
  }), _c('scroller', {
    staticClass: ["popdetail", "popdetailHeight"]
  }, _vm._l((_vm.putselect), function(item, index) {
    return _c('div', {
      key: index,
      staticClass: ["popdetailBox", "lb"]
    }, [(item.Type == 1) ? _c('text', {
      staticClass: ["lb-text1"]
    }, [_vm._v("一类")]) : (item.Type == 2) ? _c('text', {
      staticClass: ["lb-text1"]
    }, [_vm._v("二类")]) : (item.Type == 3) ? _c('text', {
      staticClass: ["lb-text1"]
    }, [_vm._v("三类")]) : (item.Type == 4) ? _c('text', {
      staticClass: ["lb-text1"]
    }, [_vm._v("四类")]) : _vm._e(), (_vm.parentItem.isEvaluate == 'N') ? _c('text', {
      staticClass: ["popdetail-text", "popdetail-text-red", "text-red", "lb-text2"]
    }, [_vm._v(_vm._s(item.TsetShortContent))]) : (_vm.parentItem.isEvaluate == 'Y') ? _c('text', {
      staticClass: ["popdetail-text", "popdetail-text-red", "text-red", "lb-text2", "lb-text2Width"]
    }, [_vm._v(_vm._s(item.TsetShortContent))]) : _vm._e(), _c('input', {
      staticClass: ["lb-text3"],
      attrs: {
        "disabled": _vm.parentItem.isEvaluate == 'Y',
        "type": "number",
        "placeholder": "金额",
        "value": (item.FillMoney)
      },
      on: {
        "input": function($event) {
          _vm.$set(item, "FillMoney", _vm._n($event.target.attr.value))
        }
      }
    }), (_vm.parentItem.isEvaluate == 'N') ? _c('text', {
      staticClass: ["close", "iconfont"],
      on: {
        "click": function($event) {
          _vm.closeList(item, index)
        }
      }
    }, [_vm._v("")]) : _vm._e()])
  }))]), (_vm.parentItem.isEvaluate == 'N') ? _c('div', {
    staticClass: ["div-c"]
  }, [_c('text', {
    staticClass: ["button-b"],
    class: [_vm.numbre1 == 5 ? 'button-gay' : ''],
    on: {
      "click": _vm.openBottomPopup
    }
  }, [_vm._v("添加违规事项")])]) : _vm._e(), _c('wxc-popup', {
    attrs: {
  ...
  }, _vm._l((_vm.popTitle), function(item, index) {
    return _c('text', {
      key: index,
      staticClass: ["poptitle-text"],
      class: [_vm.popIndex == item.Type ? 'poptitle-text-active' : ''],
      on: {
        "click": function($event) {
          _vm.selectTop(item, index)
        }
      }
    }, [_vm._v(_vm._s(item.TypeName))])
  })), _c('scroller', {
    staticClass: ["popdetail"]
  }, [_c('div', {
    staticClass: ["popdetailBox"]
  }, _vm._l((_vm.popDetailAll), function(item, index) {
    return (item.Type == _vm.popIndex) ? _c('text', {
      key: index,
      staticClass: ["popdetail-text"],
      class: [!item.checked ? 'popDisabled' : ''],
      on: {
        "click": function($event) {
          _vm.selectDetail(item, index)
        }
      }
    }, [_vm._v(_vm._s(item.TsetShortContent))]) : _vm._e()
  }))])])]), (_vm.parentItem.isEvaluate == 'N') ? _c('div', {
    staticClass: ["veh-bottom"]
  }, [(_vm.numbre1 == 5 || _vm.putselect.length > 0) ? _c('div', {
