...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/factorymanager/FMPassword.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-049eb27c"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  name: 'FMPassword',
  components: { WxcMinibar: _weexUi.WxcMinibar, WxcCell: _weexUi.WxcCell },
  data: function data() {
    return {
      oldPassword: '',
      newPassword: '',
  ...
  methods: {
    ...
    minibarRightButtonClick: function minibarRightButtonClick() {
      // 密码判断
      if (this.oldPassword == "") {
        modal.toast({ message: "旧密码不能为空" });
        return;
      } else if (this.newPassword == "") {
        modal.toast({ message: "新密码不能为空" });
        return;
      } else if (this.commitPassword == "") {
        modal.toast({ message: "确认密码不能为空" });
        return;
      } else if (this.newPassword !== this.commitPassword) {
        modal.toast({ message: "新密码不一致" });
        return;
      }

      // 保存密码  => sUserType 司机为1，其他为0
      stream.fetch({
        method: 'POST',
        url: app_server_url + 'ChangeAppPassword?sUserid=' + this.sUserID + '&oldpass=' + this.oldPassword + '&newpass=' + this.newPassword + '&sUserType=' + this.sUserType,
        type: 'json'
      }, function (ret) {
        // 判断密码
  ...
  created: function created() {
    ...
    // 获取用户修改密码类型
    storage.getItem("sUserType", function (event) {
      if (event.data === "2") {
        _this.sUserType = "1";
      } else {
        _this.sUserType = "0";
  ...
  return _c('div', {
    ...
    on: {
      "input": function($event) {
        _vm.oldPassword = $event.target.attr.value
      }
    },
    slot: "value"
  })]), _c('wxc-cell', {
    ...
    on: {
      "input": function($event) {
        _vm.newPassword = $event.target.attr.value
      }
    },
    slot: "value"
  })]), _c('wxc-cell', {
    ...
    attrs: {
      ...
      "value": (_vm.commitPassword)
    },
    on: {
      "input": function($event) {
        _vm.commitPassword = $event.target.attr.value
      }
    },
    slot: "value"
  })])], 1)
