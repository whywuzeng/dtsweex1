...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/driver/DriverScanning.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-7cbae8a4"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  ...
  created: function created() {
    ...
    storage.getItem("driverIndex", function (event) {
      // modal.toast({ message: "司机:"+JSON.parse(event.data).DriverID });
      _this.lodinisShow = true;
      stream.fetch({
        method: "POST",
        url: app_server_url + "APPDriverList/GetDriverBaseList?DriverID=" + JSON.parse(event.data).DriverID + "&FactoryID=" + JSON.parse(event.data).factoryid + "&TYPE=13",
        ...
        if (ret.ok === false) {
          ...
          _this.src = ret.data.Data.DriverBaseList[0].Code;
          if (_this.src.length > 0) _this.onstart();
        }
      });
    });
  },
