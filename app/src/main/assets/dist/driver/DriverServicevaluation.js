...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/driver/DriverServicevaluation.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-ddea792e"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  name: "DriverServicevaluation",
  components: { WxcButton: _weexUi.WxcButton, WxcMinibar: _weexUi.WxcMinibar, WxcLoading: _weexUi.WxcLoading, WxcPartLoading: _weexUi.WxcPartLoading, Utils: _weexUi.Utils },
  data: function data() {
    return {
      lodinisShow: false,
      lists1: [0, 0, 0, 0, 0],
  ...
  methods: {
    ...
    start1: function start1(index) {
      this.numbre1 = index + 1;
      this.lists1 = [0, 0, 0, 0, 0];
      for (var i = 0; i <= index; i++) {
        this.lists1[i] = 1;
      }
      // modal.toast({ message: "星星1: " + this.numbre1 });
    },
    start2: function start2(index) {
      this.numbre2 = index + 1;
      this.lists2 = [0, 0, 0, 0, 0];
      for (var i = 0; i <= index; i++) {
        this.lists2[i] = 1;
      }
      // modal.toast({ message: "星星2: " + this.numbre2 });
    },
    start3: function start3(index) {
      this.numbre3 = index + 1;
      this.lists3 = [0, 0, 0, 0, 0];
      for (var i = 0; i <= index; i++) {
        this.lists3[i] = 1;
      }
      // modal.toast({ message: "星星3: " + this.numbre3 });
    },
    wxcButtonClicked: function wxcButtonClicked(e) {
      var _this2 = this;

      // modal.toast({ message: "id: " + this.remarks +"星星3: " + this.numbre3});

      if (this.numbre1 == 0) {
        modal.toast({ message: "请给满意度评分" });
        return;
      }
      if (this.numbre2 == 0) {
        modal.toast({ message: "请给效率值评分" });
        return;
      }
      if (this.numbre3 == 0) {
        modal.toast({ message: "请给安全评分" });
        return;
      }
      var urls = "";
      ...
      storage.getItem("objbill", function (event) {
        ...
        if (_this2.remarks == undefined || _this2.remarks == "undefined" || _this2.remarks == "") {
          urls = app_server_url + "APPDriverList/Driver_Evaluate_Save?DriverID=" + JSON.parse(event.data).DriverID + "&bill_id=" + JSON.parse(event.data).bill_id + "&num1=" + _this2.numbre1 + "&num2=" + _this2.numbre2 + "&num3=" + _this2.numbre3;
        } else {
          _this2.remarks = encodeURIComponent(_this2.remarks);
          urls = app_server_url + "APPDriverList/Driver_Evaluate_Save?DriverID=" + JSON.parse(event.data).DriverID + "&bill_id=" + JSON.parse(event.data).bill_id + "&num1=" + _this2.numbre1 + "&num2=" + _this2.numbre2 + "&num3=" + _this2.numbre3 + "&remarks=" + _this2.remarks;
        }
        _this2.lodinisShow = true;
        stream.fetch({
          method: "POST",
          url: urls,
          ...
          if (ret.ok === false) {
            ...
            if (parseInt(ret.data.Data.EvaluateReturn[0].ai_value) == 1) {
              var num = _this2.$getConfig().bundleUrl.indexOf("DriverServicevaluation.js");
              ...
              navigator.push({
                ...
                // modal.toast({ message: "完成跳转" })
                // navigator.pop({}, e => {});
              });
            } else {
              modal.toast({
                message: ret.data.Data.EvaluateReturn[0].as_msg
              });
            }
          }
        });
  ...
  created: function created() {
    ...
    var bill_id = "";
    var DriverID = "";
    storage.getItem("isService", function (event) {
      DriverID = JSON.parse(event.data).DriverID;

      bill_id = JSON.parse(event.data).bill_id;

      if (DriverID.length > 0 && DriverID != undefined) {
        _this3.lodinisShow = true;
        stream.fetch({
          method: "POST",
          url: app_server_url + "APPDriverList/GetDriver_Evaluate?DriverID=" + DriverID + "&bill_id=" + bill_id,
          // url: `${app_server_url}APPDriverList/Driver_Security_Select?DriverID=${this.driverID}&shipid=${this.FactoryID}`,
          type: "json",
          timeout: "30000"
        }, function (ret) {
          ...
          if (ret.ok === false) {
            ...
            if (parseInt(ret.data.Data.DriverEvaluateList.length) > 0) {
              ret.data.Data.DriverEvaluateList.forEach(function (element, index) {
                if (index == 0) {
                  _this.start3(parseInt(element.evaluate_num3) - 1);
                  _this.start2(parseInt(element.evaluate_num2) - 1);
                  _this.start1(parseInt(element.evaluate_num) - 1);
                  _this.remarks = element.evaluate_remarks;
                }
              });
            } else {
              // modal.toast({
              //   message: ret.data.Data.DriverEvaluateList[0].as_msg
              // });
            }
          }
        });
    ...
    //  modal.toast({ message: "高:" + Utils.env.getPageHeight() });
  }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
  ...
  }, [_vm._v("装卸效率")]), _vm._l((_vm.lists1), function(item, index) {
    return _c('div', {
      key: index,
      staticClass: ["div-x"],
      on: {
      ...
      staticClass: ["text-h", "iconfont", "x-bkcl"]
    }, [_vm._v("")]) : _c('text', {
    ...
    staticClass: ["div-text"]
  }, [_vm._v("整体效率")]), _vm._l((_vm.lists2), function(item, index) {
    return _c('div', {
      key: index,
      staticClass: ["div-x"],
      on: {
    ...
    staticClass: ["div-text"]
  }, [_vm._v("工作态度")]), _vm._l((_vm.lists3), function(item, index) {
    return _c('div', {
      key: index,
      staticClass: ["div-x"],
      on: {
  ...
  return _c('div', {
    ...
    attrs: {
      "resize": "stretch",
      "src": "https://tt.ab-inbev.cn/TrackApp/AppUpdate/dtsappimages/fuwupinjia.png"
    }
  })])
}]}
module.exports.render._withStripped = true
