...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/driver/DriverAbnormal.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-09170876"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
var _DriverAbnormaltab = __webpack_require__(274);

var _DriverAbnormaltab2 = _interopRequireDefault(_DriverAbnormaltab);
...
exports.default = {
  ...
  components: {
    ...
    WxcTag: _weexUi.WxcTag,
    WxcGridSelect: _weexUi.WxcGridSelect,
    Utils: _weexUi.Utils
  },
  data: function data() {
    return {
      ...
      number: "0",
      tabList: [1, 2],
      tabTitles: _DriverAbnormaltab2.default.tabTitles,
      tabStyles: _DriverAbnormaltab2.default.tabStyles,
  ...
  methods: _defineProperty({
    nextAdd: function nextAdd() {
      ...
      this.lists.forEach(function (element) {
        _this2.pickeList.push(element.SKUName);
      });
      picker.pick({
        height: "500px",
        items: _this.pickeList
      }, function (ret) {
        var result = ret.result;
        if (result == "success") {
          // modal.toast({ message: ret.data });
          // _this.listsSku = [];
          _this.lists.forEach(function (element) {
            if (element.SKUName == _this.pickeList[ret.data]) {
              // modal.toast({
              //   message:  "==/==" + element.code
              // });
              if (_this.listsSku.length > 0) _this.listsSku.forEach(function (item) {
                if (item.SKUName != element.SKUName) _this.listsSku.push(element);
              });else _this.listsSku.push(element);
            }
          });
        }
      });
    },
    nextEx: function nextEx(item) {
      var _this3 = this;

      var _this = this;
      this.pickeList = [];

      this.testData1.forEach(function (element) {
        _this3.pickeList.push(element.title);
      });
      picker.pick({
        height: "500px",
        items: _this.pickeList
        ...
        if (result == "success") {
          modal.toast({ message: ret.data });
          _this.testData1.forEach(function (element) {
            if (element.title == _this.pickeList[ret.data]) {
              item.abnormal = _this.pickeList[ret.data];
            }
          });
        }
      });
    },
    nextXz: function nextXz() {
      var _this = this;
      _this.pickeList = [];
      _this.pickeList[0] = "异常签收";
      _this.pickeList[1] = "正常签收";
      // modal.toast({ message: "请选择异常类型" });
      picker.pick({
        height: "500px",
        items: _this.pickeList
      }, function (ret) {
    ...
    viewappear: function viewappear() {
      storage.getItem("DRBackHome", function (event) {
    ...
    minibarRightButtonClick: function minibarRightButtonClick() {
      ...
      storage.getItem("objbill", function (event) {
        //  modal.toast({ message: "单号："+ JSON.parse(event.data).bill_id});
        var Longitude = JSON.parse(event.data).longitude;
        var Latitude = JSON.parse(event.data).latitude;

        if (Longitude.length == 0) Longitude = "0.000";
        if (Latitude.length == 0) Latitude = "0.000";
        if (_this4.imageName.length == 0) _this4.imageName = "null";
        if (_this4.isImgtj.length > 0) {
          if (_this4.isImgtj == 'ON') {
            modal.toast({ message: "正在上传图片稍等几秒.." });
            return;
          }
        }

        if (_this4.listsSku.length > 0) _this4.listsSku.forEach(function (element) {
          if (_this4.Bill_SKUID.length == 0) _this4.Bill_SKUID = element.Bill_SKUID;else _this4.Bill_SKUID = _this4.Bill_SKUID + "|" + element.Bill_SKUID;
          if (_this4.skuid.length == 0) _this4.skuid = element.SKUID;else _this4.skuid = _this4.skuid + "|" + element.SKUID;
          if (_this4.skunm.length == 0) {
            _this4.skunm = element.SKUnuber;
          } else {
            _this4.skunm = _this4.skunm + "|" + element.SKUnuber;
          }
          if (_this4.Detail_ID.length == 0) {
            _this4.Detail_ID = element.Detail_ID;
          } else {
            _this4.Detail_ID = _this4.Detail_ID + "|" + element.Detail_ID;
          }
          if (parseInt(element.SKUnuber) > parseInt(element.Quantity)) {
            // modal.toast({
            //   message:
            //     parseInt(element.SKUnuber) + ":" + parseInt(element.Quantity)
            // });
            _this4.isQuantity = false;
          } else {
            _this4.isQuantity = true;
          }

          _this4.testData1.forEach(function (textelement) {
            if (textelement.title == element.abnormal) {
              if (_this4.skucode.length == 0) _this4.skucode = textelement.code;else _this4.skucode = _this4.skucode + "|" + textelement.code;
            }
          });
        });
        ...
        if (_this4.isView) {
          ...
          if (_this4.money.length == 0) {
            modal.toast({ message: "请填写金额" });
            _this4.Bill_SKUID = "";
            _this4.skuid = "";
            _this4.skunm = "";
            _this4.skucode = "";
          ...
          if (_this4.imageName.length == 0) {
            _this4.imageName = "null";
            modal.toast({ message: "请拍照片" });
            _this4.Bill_SKUID = "";
            _this4.skuid = "";
            _this4.skunm = "";
            _this4.skucode = "";
          ...
          if (_this4.Bill_SKUID.length == 0 && _this4.Detail_ID.length == 0) {
            modal.toast({ message: "物料信息为空请选择" });
            _this4.Bill_SKUID = "";
            _this4.skuid = "";
            _this4.skunm = "";
            _this4.skucode = "";
          ...
          if (!_this4.isQuantity) {
            modal.toast({ message: "SKU 数量过多" });
            _this4.Bill_SKUID = "";
            _this4.skuid = "";
            _this4.skunm = "";
            _this4.skucode = "";
        ...
        if (JSON.parse(event.data).bill_id_sub.length == 0) {
          modal.toast({ message: "没有经销商数据，请重试" });
          return;
        }

        var urls = "";
        if (_this4.isView && _this4.Bill_SKUID.length > 0 && _this4.Detail_ID.length > 0) {
          _this4.Bill_SKUID = encodeURIComponent(_this4.Bill_SKUID);
          _this4.skuid = encodeURIComponent(_this4.skuid);
          _this4.skunm = encodeURIComponent(_this4.skunm);
          _this4.skucode = encodeURIComponent(_this4.skucode);
          _this4.Detail_ID = encodeURIComponent(_this4.Detail_ID);

          if (JSON.parse(event.data).StatusType == "04") {
            urls = app_server_url + "APPDriverList/Driver_AbnormalPhoto_Save_NEW_TWO?DriverID=" + JSON.parse(event.data).DriverID + "&bill_id=" + JSON.parse(event.data).bill_id + "&Code=" + _this4.skucode + "&Longitude=" + (Longitude + "") + "&Latitude=" + (Latitude + "") + "&path=" + _this4.imageName + "&ship_to=" + JSON.parse(event.data).bill_id_sub + "&skuid=" + _this4.skuid + "&skunum=" + _this4.skunm + "&money=" + _this4.money + "&detail_id=" + _this4.Detail_ID + "&shipid=" + _this4.ShipmentId + "&detail_skuid=" + _this4.Bill_SKUID;
          } else {

            urls = app_server_url + "APPDriverList/Driver_AbnormalPhoto_Save_NEW?DriverID=" + JSON.parse(event.data).DriverID + "&bill_id=" + JSON.parse(event.data).bill_id + "&Code=" + _this4.skucode + "&Longitude=" + (Longitude + "") + "&Latitude=" + (Latitude + "") + "&path=" + _this4.imageName + "&ship_to=" + JSON.parse(event.data).bill_id_sub + "&skuid=" + _this4.skuid + "&skunum=" + _this4.skunm + "&money=" + _this4.money + "&detail_id=" + _this4.Detail_ID + "&shipid=" + _this4.ShipmentId + "&detail_skuid=" + _this4.Bill_SKUID;
          }
        } else if (JSON.parse(event.data).StatusType == "04") {
          urls = app_server_url + "APPDriverList/Driver_AbnormalPhoto_Save_NEW_TWO?DriverID=" + JSON.parse(event.data).DriverID + "&bill_id=" + JSON.parse(event.data).bill_id + "&Longitude=" + Longitude + "&Latitude=" + Latitude + "&QueueID=null&bill_id_sub=" + JSON.parse(event.data).bill_id_sub + "&DetailID=null";
        } else urls = app_server_url + "APPDriverList/Driver_Factory_Register?DriverID=" + JSON.parse(event.data).DriverID + "&bill_id=" + JSON.parse(event.data).bill_id + "&Longitude=" + Longitude + "&Latitude=" + Latitude + "&QueueID=null&bill_id_sub=" + JSON.parse(event.data).bill_id_sub + "&DetailID=null";
        _this4.lodinisShow = true;
        // modal.toast({ message:  urls})
        stream.fetch({
          method: "POST",
          url: urls,
          ...
          if (ret.ok === false) {
            ...
            if (parseInt(ret.data.Data.BillStatusReturn[0].ai_value) == 1) {
              if (parseInt(_this4.number) > 0) {
                _this4.lodinisShow = true;
                stream.fetch({
                  method: "POST",
                  url: app_server_url + "APPDriverList/Driver_Bottle_Save?DriverID=" + JSON.parse(event.data).DriverID + "&shipid=" + JSON.parse(event.data).bill_id_sub + "&number=" + _this4.number + "&bill_id=" + JSON.parse(event.data).bill_id,
                  // url: `${app_server_url}APPDriverList/Driver_Bottle_Save?DriverID=${this.driverID}&shipid=${this.shipid}&number=${this.number}`,
                  type: "json",
                  timeout: "60000"
                }, function (ret) {
                  _this4.lodinisShow = false;
                  if (ret.ok === false) {
                    ...
                    if (parseInt(ret.data.Data.BillStatusReturn[0].ai_value) == 1) {
                      var num = _this4.$getConfig().bundleUrl.indexOf("DriverAbnormal.js");
                      var bundleUrl = _this4.$getConfig().bundleUrl.substr(0, num);

                      // modal.toast({ message:  bundleUrl+"/"+num})
                      navigator.push({
                        url: bundleUrl + "DriverResult.js",
                        animated: "true"
                      }, function (event) {});
                      modal.toast({
        ...
        // modal.toast({ message: "数量：" + this.number });
      });
    },
    uploadByNative: function uploadByNative(filePath) {
      ...
      var _this = this;
      _this.isImgtj = "ON";
      // modal.toast({ message: "上传：" + filePath });
      this.lodinisShow = true;
      (0, _DTSPlugins.YtImageUploader)({
        imagePath: filePath,
        serverUrl: app_server_url + "APPDriverList/UploadPhoto"
        //"http://139.219.133.22/DTSAPI/APPDriverList/UploadPhoto"
      }, function (err, ret) {
        _this5.lodinisShow = false;
        if (err === null || err === undefined) {
          _this5.imageName = ret.imageName;
          modal.toast({
            message: "图片上传上传成功"
          });
          _this.isImgtj = "";
      ...
      // Nat.upload(
      //   "https://sfatest.unidms.com/DTSApi/APPDriverList/UploadPhoto",
      //   {
      //     path: filepath
      //   },
      //   (err, ret) => {
      //     console.log(ret);
      //     modal.toast({ message: "上传:" + ret });
      //   }
      // );
      // nat_network_transfer.upload(
      //   {
      //     url: "https://sfatest.unidms.com/DTSApi/APPDriverList/UploadPhoto",
      //     path: filePath
      //   },
      //   result => {
      ...
      //     if (progress) {
      //       modal.toast({ message: "上传中!" });
      //       return;
      //     }
      //     if (!ok) {
      //       modal.toast({ message: "上传失败!" });
      //       return;
      //     } else {
    ...
    takePhoto: function takePhoto() {
      ...
      (0, _DTSPlugins.CaptureImage)({}, function (err, ret) {
        if (err === null || err === undefined) {
          // modal.toast({ message: "photoPaht=" + ret.path });
          _this6.imgurl = ret.path;
          _this6.logoUrl = ret.path;
          _this6.uploadByNative(ret.path);
        }
      });
    },
    onSelect1: function onSelect1(_ref) {
      var selectIndex = _ref.selectIndex,
          checked = _ref.checked,
          checkedList = _ref.checkedList;

      this.code = checkedList.map(function (item) {
        return item.code;
      });
      // modal.toast({ message: "onSelect1:" + this.code });
    },
    wxcButtonClicked: function wxcButtonClicked() {
      // modal.toast({ message:"wxcButtonClicked:"+ this.code });
    },
    poshttp: function poshttp() {
      //  modal.toast({ message: "poshttp"});
      var _this = this;
      storage.getItem("objbill", function (event) {
        _this.ordeid = JSON.parse(event.data).bill_id;
        _this.jName = JSON.parse(event.data).jName;
        // modal.toast({ message: "司机id："+_this.jName});
        _this.lodinisShow = true;

        stream.fetch({
          method: "POST",
          url: app_server_url + "APPDriverList/GetDriverBILL_Detail_List?DriverID=" + JSON.parse(event.data).DriverID + "&TYPE=01&Code=" + _this.ordeid + "&DestinationId=" + JSON.parse(event.data).bill_id_sub,
          // url: `${app_server_url}APPDriverList/Driver_Security_Select?DriverID=${this.driverID}&shipid=${this.FactoryID}`,
          type: "json"
        }, function (ret) {
          ...
          if (ret.ok === false) {
            ...
            ret.data.Data.DriverDetailList.forEach(function (element, index) {
              if (index == 0) {
                _this.ShipmentId = element.ShipmentId;
              }
              // modal.toast({ message: "stream" });
            });
            _this.lodinisShow = true;
            stream.fetch({
              method: "POST",
              url: app_server_url + "APPDriverList/GetDriverBaseList?DriverID=" + JSON.parse(event.data).DriverID + "&TYPE=06",
              // url: `${app_server_url}APPDriverList/Driver_Security_Select?DriverID=${this.driverID}&shipid=${this.FactoryID}`,
              type: "json"
            }, function (ret) {
              _this.lodinisShow = false;
              if (ret.ok === false) {
                ...
                ret.data.Data.DriverBaseList.forEach(function (element) {
                  element.title = element.Name;
                  element.checked = false;
                  element.code = element.Code;
                  _this.testData1.push(element);
                  // modal.toast({ message: "stream" });
                });
                _this.lodinisShow = true;
                stream.fetch({
                  method: "POST",
                  url: app_server_url + "APPDriverList/GetDriverBILL_Detail_SKU_new?DriverID=" + JSON.parse(event.data).DriverID + "&Code=" + _this.ordeid + "&shipid=" + _this.ShipmentId + "&TYPE=01",
                  // url: `${app_server_url}APPDriverList/Driver_Security_Select?DriverID=${this.driverID}&shipid=${this.FactoryID}`,
                  type: "json"
                }, function (ret1) {
                  _this.lodinisShow = false;
                  if (ret1.ok === false) {
                    ...
                    if (ret1.data.Data.DriverEvaluateList.length > 0) ret1.data.Data.DriverEvaluateList.forEach(function (element) {
                      element.SKUnuber = 1;
                      element.abnormal = _this.testData1[0].title;
                      _this.lists.push(element);
                    });
                    // modal.toast({ message: this.lists.DestinationName });
                  }
                });
              }
            });
  ...
  created: function created() {
    ...
    _this.tabPageHeight = _weexUi.Utils.env.getPageHeight();
    _this.contentStyle = { height: _weexUi.Utils.env.getPageHeight() + "px" };
    _this.poshttp();
  }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    return _c('div', {
      ...
      style: _vm.contentStyle
    }, [(index == 0) ? _c('div', [_c('div', {
      staticClass: ["div-top-xz"],
      on: {
        "click": _vm.nextXz
      }
    }, [_c('text', {
      staticClass: ["text-top-xz"]
    }, [_vm._v("请选择签收类型 (选择后显示具体类型)  " + _vm._s(_vm.qsstaic))]), _c('text', {
      staticClass: ["text-top-xz-rig", "iconfont"]
    }, [_vm._v("")])]), _c('div', {
      staticClass: ["div-lin"]
    }), _c('div', {
      staticClass: ["div-top"]
      ...
      staticClass: ["top-text"]
    }, [_vm._v(_vm._s(_vm.ShipmentId))])]), (_vm.isView) ? _c('div', {
      staticClass: ["div-top"]
    }, [_c('text', {
      staticClass: ["top-text"]
    }, [_vm._v("赔偿金额:  ")]), _c('input', {
      ...
      on: {
        "input": function($event) {
          _vm.money = $event.target.attr.value
        }
      }
      ...
      staticClass: ["c-txt-list"]
    }, [_vm._v("添加物料")])]) : _vm._e(), (_vm.isView) ? _c('div', {
      staticClass: ["div-b-list"]
    }, [_c('text', {
      staticClass: ["b-txt-list"]
    }, [_vm._v("SKU")]), _c('text', {
      staticClass: ["b-txt-list"]
    }, [_vm._v("异常原因")]), _c('text', {
      staticClass: ["b-txt-list"]
    }, [_vm._v("数量")])]) : _vm._e(), (_vm.isView) ? _c('list', {
      staticClass: ["div-list-main"]
    }, _vm._l((_vm.listsSku), function(item, index) {
      return _c('cell', {
        key: index,
        staticClass: ["cell"],
        appendAsTree: true,
        ...
        staticClass: ["txt-list"]
      }, [_vm._v(_vm._s(item.SKUName))]), _c('text', {
        staticClass: ["txt-list"],
        on: {
          "click": function($event) {
            _vm.nextEx(item)
      ...
      }, [_vm._v(_vm._s(item.abnormal))]), _c('input', {
        ...
        on: {
          "blur": function($event) {
            item.SKUnuber = parseInt(item.SKUnuber)
          },
          "input": function($event) {
            _vm.$set(item, "SKUnuber", $event.target.attr.value)
          }
        }
      })])])
    })) : _vm._e(), (_vm.isView) ? _c('div', {
      staticClass: ["div-context"]
    }, [_c('div', {
      staticClass: ["div-lin"]
    }), _c('div', [_c('image', {
      staticClass: ["div-img"],
      ...
      attrs: {
        "resize": "stretch",
        "src": "https://tt.ab-inbev.cn/TrackApp/AppUpdate/dtsappimages/siji-paizhao.png"
      },
      on: {
        "click": _vm.takePhoto
      }
    ...
    }) : _vm._e()], 1)]) : _vm._e()]) : _vm._e(), (index == 1) ? _c('div', [_c('div', {
      staticClass: ["div-t"]
    }, [_c('text', {
      staticClass: ["text-top"]
    }, [_vm._v("请根据客户提供的信息妥善填写")])]), _c('div', {
      staticClass: ["div-bm"]
    }, [_c('div', {
      staticClass: ["m-cell"]
    }, [_c('div', {
      ...
      on: {
        "input": function($event) {
          _vm.jName = $event.target.attr.value
        }
      }
    })]), _c('div', {
      staticClass: ["border-bottom"]
      ...
      attrs: {
        "type": "number",
        "placeholder": "请准确填写对托数",
        "value": "0",
        "value": (_vm.number)
      },
      on: {
        "input": function($event) {
          _vm.number = $event.target.attr.value
        }
      }
    })]), _c('div', {
      staticClass: ["border-bottom"]
