...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/driver/DriverAnswer.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-1075997b"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  ...
  methods: {
    ...
    wxcCheckBoxListChecked: function wxcCheckBoxListChecked(e) {
      ...
      // modal.toast({ message: "checkedList:="+this.checkedList.length+"/=="+this.list.length });
    },
    wxcButtonClicked: function wxcButtonClicked(e) {
      ...
      if (this.checkedList.length != this.list.length) {
        modal.toast({
          message: "请仔细阅读，且全选！(温馨提示可以往下滑动查看是否还有题目未勾选)"
        });
        return;
      }
      ...
      storage.getItem("driverfactoryid", function (event) {
        //  modal.toast({ message: "工厂id:"+this.FactoryID+"司机="+this.driverID });
        stream.fetch({
          method: "POST",
          url: app_server_url + "APPDriverList/Driver_Security_Save?DriverID=" + _this2.driverID + "&FactoryID=" + _this2.FactoryID + "&Code=1",
          ...
          if (ret.ok === false) {
            ...
            if (parseInt(ret.data.Data.BillStatusReturn[0].ai_value) == 1) {
              var num = _this2.$getConfig().bundleUrl.indexOf("DriverAnswer.js");
              var bundleUrl = _this2.$getConfig().bundleUrl.substr(0, num);

              // modal.toast({ message:  bundleUrl+"/"+num})
              navigator.push({
                url: bundleUrl + "DriverABRegist.js",
                animated: "true"
              }, function (event) {});
              //  navigator.pop({}, e => {});
            } else {
  ...
  created: function created() {
    ...
    this.contentStyle = { height: scrollHeight + "px" };
    this.scrollVh = { height: scrollHeight - 90 + "px" };
    storage.getItem("objbill", function (event) {
      ...
      //  modal.toast({ message:  _this.driverID});
      stream.fetch({
        method: "POST",
        url: app_server_url + "APPDriverList/Driver_Security_Select?DriverID=" + _this.driverID + "&FactoryID=1",
        // url: `${app_server_url}APPDriverList/Driver_Security_Select?DriverID=${this.driverID}&shipid=${this.FactoryID}`,
        type: "json"
      }, function (ret) {
        if (ret.ok === false) {
          ...
          ret.data.Data.DriverList.forEach(function (val) {
            val.title = val.Name;
            val.value = val.Code;
            val.checked = false;
          });
          _this3.list = ret.data.Data.DriverList;
          // modal.toast({ message: JSON.stringify(this.list) });
        }
      });
    });
  }
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["div-bott"]
  }, [(_vm.checkedList.length == _vm.list.length) ? _c('wxc-button', {
