...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/driver/DriverBackbottle.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-35fd35ac"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  name: "DriverBackbottle",
  components: { WxcButton: _weexUi.WxcButton, WxcMinibar: _weexUi.WxcMinibar, WxcCell: _weexUi.WxcCell, WxcLoading: _weexUi.WxcLoading, WxcPartLoading: _weexUi.WxcPartLoading },
  data: function data() {
    return {
      driverID: "",
      shipidCode: "",
  ...
  methods: {
    nextDriverTruckid: function nextDriverTruckid() {
      var isService = {};
      isService.DriverID = this.driverID;
      // modal.toast({ message:  this.driverID });
      isService.type = "Truckid";
      storage.setItem("isService", JSON.stringify(isService));
      var num = this.$getConfig().bundleUrl.indexOf("DriverBackbottle.js");
      var bundleUrl = this.$getConfig().bundleUrl.substr(0, num);

      // modal.toast({ message:  bundleUrl+"/"+num})
      navigator.push({
        url: bundleUrl + "DriverSelect.js",
        animated: "true"
      }, function (event) {});
    },
    nextDriverShipid: function nextDriverShipid() {
      var isService = {};
      isService.DriverID = this.driverID;
      // modal.toast({ message:   "nextDriverShipid:"+isService.DriverID });
      isService.type = "Shipid";
      storage.setItem("isService", JSON.stringify(isService));
      var num = this.$getConfig().bundleUrl.indexOf("DriverBackbottle.js");
      var bundleUrl = this.$getConfig().bundleUrl.substr(0, num);
    ...
    viewappear: function viewappear() {
      ...
      storage.getItem("DRBackHome", function (event) {
        if (event.result == "success") {
          ...
          storage.getItem("bottleData", function (event) {
            _this.driverID = JSON.parse(event.data).DriverID;

            if (JSON.parse(event.data).type == "Truckid") {
              _this.truckid = JSON.parse(event.data).Name;
              _this.truckidCode = JSON.parse(event.data).Code + "";
            } else {
              _this.shipid = JSON.parse(event.data).Name;
              _this.shipidCode = JSON.parse(event.data).Code;
            }
            if (_this.shipid == undefined) {
              _this.shipid = "请选择经销商";
            }
            if (_this.truckid == undefined) {
              _this.truckid = "请选择车牌";
            }
          });
        }
    ...
    wxcButtonClicked: function wxcButtonClicked(e) {
      ...
      if (this.shipidCode === undefined || this.shipidCode === "") {
        modal.toast({ message: "请选择经销商" });
        return;
      }
      if (this.truckidCode === undefined || this.truckidCode === "") {
        modal.toast({ message: "请选择车辆" });
        return;
      }
      if (this.driverID === undefined || this.driverID === "") {
        modal.toast({ message: "请填写客户名称" });
        return;
      } else if (this.number === undefined || this.number === "" || parseInt(this.number) < 1) {
        modal.toast({ message: "请准确填写对托数" });
        return;
      }
      this.lodinisShow = true;
      //
      storage.getItem("DriverID", function (event) {
        _this2.driverID = event.data;
        stream.fetch({
          method: "POST",
          url: app_server_url + "APPDriverList/Driver_Bottle_SaveTwo?DriverID=" + _this2.driverID + "&shipid=" + _this2.shipidCode + "&truckid=" + _this2.truckidCode + "&number=" + (_this2.number + ""),
          ...
          if (ret.ok === false) {
            ...
            if (parseInt(ret.data.Data.BillStatusReturn[0].ai_value) == 1) {
              var num = _this2.$getConfig().bundleUrl.indexOf("DriverBackbottle.js");
              var bundleUrl = _this2.$getConfig().bundleUrl.substr(0, num);

              // modal.toast({ message:  bundleUrl+"/"+num})
              storage.setItem("bottleData", "", function (event) {});
              navigator.push({
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    on: {
      "wxcCellClicked": _vm.nextDriverShipid
    }
  }, [_c('text', {
  ...
  }, [_vm._v(_vm._s(_vm.shipid))])]), _c('wxc-cell', {
    attrs: {
      "title": "车辆",
      "hasArrow": true,
      "hasTopBorder": false
    ...
    on: {
      "wxcCellClicked": _vm.nextDriverTruckid
    }
  }, [_c('text', {
    staticClass: ["right-text"],
    staticStyle: {
  ...
  }, [_vm._v(_vm._s(_vm.truckid))])]), _vm._m(0), _c('wxc-loading', {
    attrs: {
      "needMask": true,
  ...
  }, [_vm._v(_vm._s(_vm.shipid))])]), _c('div', {
    staticClass: ["border-bottom"]
  }), _c('div', {
    staticClass: ["cell-item"]
  }, [_c('input', {
