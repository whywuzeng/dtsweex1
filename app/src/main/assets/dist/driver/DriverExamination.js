...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/driver/DriverExamination.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-4ea401f8"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  name: "DriverExamination",
  components: {
    Title: _weexUi.Title,
    Category: _weexUi.Category,
    WxcCheckbox: _weexUi.WxcCheckbox,
    WxcCheckboxList: _weexUi.WxcCheckboxList,
  ...
  created: function created() {
    ...
    storage.getItem("objbill", function (event) {
      _this.factoryid = JSON.parse(event.data).factoryid;
      // _this.factoryid = "WUH";
      _this.DriverID = JSON.parse(event.data).DriverID;
      //  modal.toast({ message: "司机："+_this.factoryid+"/:"+_this.DriverID});
      _this.lodinisShow = true; //数据加载框
      stream.fetch({
        method: "POST",
        url: app_server_url + "QuestionList/GetQuestionList?DriverID=" + _this.DriverID + "&FactoryID=" + _this.factoryid,
        type: "json"
      }, function (ret) {
        ...
        if (ret.ok === false) {
          ...
          _this.titlelist = [];
          _this.titlelist = ret.data.Data.QuestionList;

          if (_this.titlelist.length > 0) {

            if (_this.titlelist[0].AnswerType == "0") {
              _this.isChoxs = "N";
              _this.isImgs = "N";
            } else if (_this.titlelist[0].AnswerType == "1") {
              _this.isChoxs = "Y";
              _this.isImgs = "N";
            } else if (_this.titlelist[0].AnswerType == "11") {
              _this.isChoxs = "N";
              _this.isImgs = "Y";
              _this.titlelist[0].QuestionContent = _this.titlelist[0].QuestionContent + "(点击图片查看大图)";
            }

            _this.optitle = _this.titlelist[0].QuestionContent;
            _this.list = [];

            _this.titlelist.forEach(function (element) {
              if (element.AnswerType == "11") {
                element.QuestionContent = element.QuestionContent + "(点击图片查看大图)";
              }
              element.AnswerList.forEach(function (elementcontext) {
                elementcontext.checked = false;
                if (_this.titlelist[0].QuestionID == elementcontext.QuestionID) {
                  elementcontext.value = elementcontext.score;
                  elementcontext.title = elementcontext.OptionCode + " " + elementcontext.OptionContent;
                  _this.list.push(elementcontext);
                }
              });
            });

            _this.maxIndex = _this.titlelist.length;
          } else {
            modal.toast({ message: "暂无相关考试信息" });
          }

          // datalist.fetch(element => {
          // _this.titlelist.push(element);
          // });

          //  modal.toast({ message: _this.titlelist[0].QuestionContent });
        }
      });
    });
  },
  ...
  methods: {
    ...
    // 上一题
    preBtnClick: function preBtnClick() {
      var _this = this;

      this.indexs--;

      _this.optitle = _this.titlelist[_this.indexs - 1].QuestionContent;
      _this.myQuestionID = _this.titlelist[_this.indexs - 1].QuestionID;
      _this.imgurl = _this.titlelist[_this.indexs - 1].QuestionImage;
      _this.list = [];

      _this.titlelist.forEach(function (element, index) {
        //  modal.toast({ message: "myQuestionID" + element.QuestionID })

        element.AnswerList.forEach(function (elementcontext) {
          if (elementcontext.QuestionID == _this.myQuestionID) {
            elementcontext.value = elementcontext.score;
            elementcontext.title = elementcontext.OptionCode + " " + elementcontext.OptionContent;
            elementcontext.checked = false;
            _this.list.push(elementcontext);
      ...
      if (_this.titlelist[_this.indexs - 1].AnswerType == "0") {
        _this.isChoxs = "N";
        _this.isImgs = "N";
      } else if (_this.titlelist[_this.indexs - 1].AnswerType == "1") {
        _this.isChoxs = "Y";
        _this.isImgs = "N";
      } else if (_this.titlelist[_this.indexs - 1].AnswerType == "11") {
        _this.isChoxs = "N";
        _this.isImgs = "Y";
      }
      if (_this.indexs == _this.titlelist.length) {
        _this.buttname = "提交";
      } else {
        _this.buttname = "下一题";
      }
      ...
      // [ [{}], [{},{},{}] ]  数据结构
      var arrDate = [];
      arrDate = this.newList[this.indexs - 1];
      this.checkedInfo = "22";

      //modal.toast({message:this.indexs-1});
      //this.text = JSON.stringify(this.newList);

      if (this.isChoxs == "N") {
        // 单选
        this.list.forEach(function (val) {
          if (val.OptionContent == arrDate[0].OptionContent) {
            val.checked = true;
          }
        });
        if (Math.sign(arrDate[0].score) != -1) this.total -= arrDate[0].score;
      } else if (this.isChoxs == "Y") {
        // 多选
        this.list.forEach(function (val) {
          arrDate.forEach(function (v) {
            if (val.OptionContent == v.OptionContent) {
              val.checked = true;
            }
          });
        });
        ...
        // 多选
        var thisValue = 0;
        arrDate.forEach(function (element) {
          thisValue += element.score;
        });

        if (thisValue == _this.titlelist[_this.indexs - 1].score) {
          this.total -= thisValue;
        }
      }
    },
    ...
    bottomrightButtonClick: function bottomrightButtonClick() {
      ...
      //  modal.toast({ message: "indexs："+this.indexs+"/optindex:"+this.optindex});

      var _this = this;
      // let reg = /^\d+(?=\.{0,1}\d+$|$)/;
      //modal.toast({ message: this.isshow });
      if (this.newList[this.indexs - 1] == undefined || this.newList[this.indexs - 1] == "") {
        // 备份数据
        ...
        var arr2 = this.list.filter(function (val) {
          return val.checked;
        });

        _this.newList[this.indexs - 1] = arr2;
      }
      if (this.indexs < this.maxIndex) {
        // modal.toast({ message: "indexs："+this.indexs+"/optindex:"+this.optindex+"checkedInfo:"+this.checkedInfo.title});
        if (_this.checkedInfo == "") {
          modal.toast({ message: "请选择题目" });
          return;
        }
        if (_this.checkedLists.length == 0) {
          modal.toast({ message: "请选择题目" });
          return;
        }
        // this.arrtext=this.newList[this.indexs-1];


        if (_this.isChoxs == "N") {
          //单选
          // if(this.newList[this.indexs - 1][0].score!=-1)
          //  modal.toast({ message: "max总分：" + Math.sign(this.newList[this.indexs - 1][0].score)});
          if (Math.sign(this.newList[this.indexs - 1][0].score) != -1) {
            this.total += this.newList[this.indexs - 1][0].score;
          } else {
            this.total += 0;
          }
        } else {
          // 多选
          var thisValue = 0;
          _this.newList[this.indexs - 1].forEach(function (element) {
            thisValue += element.score;
          });
        ...
        _this.titlelist.forEach(function (element, index) {
          ...
          element.AnswerList.forEach(function (elementcontext) {
            if (elementcontext.QuestionID == _this.myQuestionID) {
              elementcontext.value = elementcontext.score;
              elementcontext.title = elementcontext.OptionCode + " " + elementcontext.OptionContent;
              elementcontext.type = element.AnswerType;
              elementcontext.checked = false;
              _this.list.push(elementcontext);
            }
          });
        ...
        if (this.newList[this.indexs - 1] != undefined && this.newList[this.indexs - 1].length > 0) {
          // 取之前的值
          arrDate = this.newList[this.indexs - 1];
          ...
          if (this.isChoxs == "N") {
            // 单选
            this.list.forEach(function (val) {
              // this.text+=this.indexs+":"+val.OptionContent+"/缓层："+arrDate[0].OptionContent;
              if (val.OptionContent == arrDate[0].OptionContent) {
                //  this.text=this.indexs+":"+val.OptionContent+"/缓层："+arrDate[0].OptionContent;
                val.checked = true;
                _this2.checkedInfo = 2;
              }
            });
          } else if (this.isChoxs == "Y") {
            // 多选
          ...
          // 备份数据
          // let arr2 = this.list.filter(val => {
          //   return val.checked;
          // });
          // this.newList[this.indexs-1] = arr;
          // this.text=arr2;

          // this.newList.splice(this.indexs - 1, 1, arr2);

          // this.newList.forEach(())
          // this.text = this.newList;
          // this.arrtext = arr2;
        } else {}

        // this.optitle=!this.newList[this.indexs-1]

        //modal.toast({message:this.indexs-1});

        // modal.toast({ message: "max总分：" + _this.total +"/"});
        // _this.list = _this.valuelist;
        ...
        if (this.isSbmt) {
          ...
          if (_this.isChoxs == "N") {
            //单选
            // if(this.newList[this.indexs - 1][0].score!=-1)
            // modal.toast({ message: "max总分：" +this.newList[this.newList.length - 1][0].score});
            if (Math.sign(this.newList[this.newList.length - 1][0].score) != -1) {
              this.total += this.newList[this.newList.length - 1][0].score;
            } else {
              this.total += 0;
            }
          } else {
            // 多选
            var _thisValue = 0;
            _this.newList[this.newList.length - 1].forEach(function (element) {
              _thisValue += element.score;
            });

            if (_thisValue == _this.titlelist[_this.titlelist.length - 1].score) {
              this.total += _thisValue;
            }
          }

          storage.getItem("driverfactoryid", function (event) {
            stream.fetch({
              method: "POST",
              url: app_server_url + "APPDriverList/Driver_Examination_Save?DriverID=" + _this.DriverID + "&FactoryID=" + _this.factoryid + "&Sum=" + _this2.total,
              ...
              if (ret.ok === false) {
                ...
                if (parseInt(ret.data.Data.BillStatusReturn[0].ai_value) == 1) {
                  var num = _this2.$getConfig().bundleUrl.indexOf("DriverExamination.js");
                  var bundleUrl = _this2.$getConfig().bundleUrl.substr(0, num);

                  navigator.push({
                    url: bundleUrl + "DriverKsResult.js",
                    animated: "true"
                  }, function (event) {});
                  //  navigator.pop({}, e => {});
                } else {
    ...
    // 单选
    readioChange: function readioChange(item) {
    ...
    wxcRadioListChecked: function wxcRadioListChecked(e) {
      //if (this.checkedInfo) this.checkedInfo = {}
      this.checkedInfo = e;
    },
    wxcCheckBoxListChecked: function wxcCheckBoxListChecked(e) {
      this.checkedLists = e.checkedList;
    }
  },
  watch: {
    list: function list(v1, v2) {
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["top-title-text"]
  }, [_vm._v("0" + _vm._s(_vm.indexs))])]), _c('div', {
    staticClass: ["div-top-title"]
  }, [_c('text', {
    staticClass: ["top-title-text-"]
  }, [_vm._v(_vm._s(_vm.optitle))])]), (_vm.isImgs == 'Y') ? _c('div', {
    staticClass: ["div-top-title"]
  }, [_c('image', {
    staticClass: ["div-img"],
    attrs: {
    ...
    staticClass: ["margin"]
  }, [(_vm.isChoxs == 'N') ? _c('div', _vm._l((_vm.list), function(item, index) {
    return _c('div', {
      key: index,
      staticClass: ["readioList"],
      on: {
        "click": function($event) {
          _vm.readioChange(item)
        }
      }
    }, [_c('text', {
      staticClass: ["readioList-text"],
      class: [item.checked ? 'red' : '']
    }, [_vm._v(_vm._s(item.title))]), (item.checked) ? _c('text', {
      staticClass: ["iconfont", "icon", "red"]
    }, [_vm._v("")]) : _c('text', {
      staticClass: ["iconfont", "icon"]
    }, [_vm._v("")])])
  })) : _vm._e(), (_vm.isChoxs == 'Y') ? _c('div', _vm._l((_vm.list), function(item, ind) {
    return _c('div', {
      key: ind,
      staticClass: ["readioList"],
      on: {
  ...
  }, [_vm._v("checkedItem: " + _vm._s(_vm.checkedInfo))])])]), _c('div', {
    staticClass: ["bottom"]
  }, [(_vm.indexs == 1) ? _c('text', {
    staticClass: ["bottom-rightButton", "galy"]
  }, [_vm._v("上一题")]) : _c('text', {
    staticClass: ["bottom-rightButton"],
    on: {
      "click": _vm.preBtnClick
    }
  }, [_vm._v("上一题")]), (_vm.checkedInfo == '') ? _c('text', {
    staticClass: ["bottom-rightButton", "galy"]
  }, [_vm._v(_vm._s(_vm.buttname))]) : _c('text', {
    staticClass: ["bottom-rightButton"],
    on: {
      "click": _vm.bottomrightButtonClick
    }
  }, [_vm._v(_vm._s(_vm.buttname))])])]), _c('wxc-mask', {
    attrs: {
      "height": "800",
      "width": "710",
      "borderRadius": "0",
