...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/driver/DriverQueueState.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-697f8c3d"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  name: "DriverQueueState",
  components: { WxcMinibar: _weexUi.WxcMinibar, WxcCell: _weexUi.WxcCell, WxcIcon: _weexUi.WxcIcon, WxcGridSelect: _weexUi.WxcGridSelect, WxcDialog: _weexUi.WxcDialog, Utils: _weexUi.Utils, WxcLoading: _weexUi.WxcLoading },
  data: function data() {
    return {
      show: false,
      hasAnimation: true,
      ...
      titlevualue: "   ",
      scrollHeight: '', //滚动的高
      //   listQ:[{'value':'1','chekvlue':false},{'value':'2','chekvlue':false},{'value':'3','chekvlue':false},{'value':'4','chekvlue':true}],
      listQ: [],
      dloglists: [],
      isTopShow: false,
      mainItem: null,
      driverIndex: { DriverID: "", type: "", drivernext: false, bill_id: "" }
    };
  },
  ...
  computed: {
    ...
    isBgcl: function isBgcl() {
      // 是否高亮
      return this.driverIndex.drivernext;
    }
  },
  methods: {
    ...
    aa: function aa() {
      var listQ = this.listQ;
      var items = null;
      listQ.forEach(function (item, index) {
        if (item.chekvlue) items = item;
      });
      this.mainItem = items;
      var index = listQ.indexOf(items);
      listQ.splice(index, 1);
      listQ.unshift(this.mainItem);
      this.listQ = listQ;
    },
    nextJf: function nextJf() {
      var _this2 = this;

      var _this = this;
      storage.getItem("objbill", function (event) {
        //   modal.toast({ message: "ID:" + event.data });
        stream.fetch({
          method: "POST",
          url: app_server_url + "APPDriverList/GetDriverBaseList?DriverID=" + JSON.parse(event.data).DriverID + "&Code=" + JSON.parse(event.data).bill_id + "&TYPE=17",
          ...
          if (ret.ok === false) {
            ...
            _this.dloglists = [];
            ret.data.Data.DriverBaseList.forEach(function (element, index) {
    ...
    minibarRightButtonClick: function minibarRightButtonClick() {
      ...
      storage.getItem("objbill", function (event) {
        ...
        stream.fetch({
          method: "POST",
          url: app_server_url + "APPDriverList/GetDriverQueueList?DriverID=" + JSON.parse(event.data).DriverID + "&Code=" + JSON.parse(event.data).bill_id + "&TYPE=01",
          // url: `${app_server_url}APPDriverList/Driver_Security_Select?DriverID=${this.driverID}&shipid=${this.FactoryID}`,
          type: "json"
        }, function (ret) {
          ...
          if (ret.ok === false) {
            ...
            _this.listQ = [];
            ret.data.Data.DriverQueueList.forEach(function (element, index) {
              element.id = JSON.parse(event.data).bill_id;
              if (JSON.parse(event.data).bill_id == element.bill_id) {
                element.chekvlue = true;

                _this3.driverIndex.drivernext = true;
              } else {
                _this3.driverIndex.drivernext = false;
                element.chekvlue = false;
              }

              _this.listQ.push(element);
              if (_this.listQ.length > 0 && JSON.parse(event.data).bill_id == element.bill_id) _this.titlevualue = _this.listQ[0].queuename;
            });
          }
        });
      });
        ...
        if (ret.ok === false) {
          ...
          ret.data.Data.DriverQueueList.forEach(function (element, index) {
            ...
            if (JSON.parse(event.data).bill_id == element.bill_id) {
              ...
              _this4.driverIndex.drivernext = true;
            } else {
              _this4.driverIndex.drivernext = false;
              element.chekvlue = false;
            }
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
  ...
  }, [_vm._v(_vm._s(_vm.titlevualue))])]), _c('text', {
    staticClass: ["queueF-right"],
    on: {
      "click": _vm.nextJf
    }
  }, [_vm._v("计分标准")])]), _vm._m(1), _c('scroller', {
    ...
    return _c('div', {
      key: index,
      class: [item.id == item.bill_id ? 'veh-header-rg' : 'veh-header']
    }, [_c('text', {
      class: [item.id == item.bill_id ? 'veh-header-text-rg' : 'veh-header-text']
      ...
      class: [item.id == item.bill_id ? 'veh-header-text-rg' : 'veh-header-text']
    }, [_vm._v(_vm._s(item.status))]), _c('text', {
      class: [item.id == item.bill_id ? 'veh-header-text-rg' : 'veh-header-text']
    }, [_vm._v(_vm._s(item.callTime))]), _c('text', {
      class: [item.id == item.bill_id ? 'veh-header-text-rg' : 'veh-header-text']
