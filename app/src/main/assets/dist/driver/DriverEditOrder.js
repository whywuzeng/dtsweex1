...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/driver/DriverEditOrder.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-71eb1bb7"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  name: "DriverEditOrder",
  components: { WxcButton: _weexUi.WxcButton, WxcMinibar: _weexUi.WxcMinibar },
  data: function data() {
    return {
      checklist: [],
      list: [],
  ...
  methods: {
    ...
    bottomrightButtonClick: function bottomrightButtonClick() {
      ...
      if (this.checklist.length == 0) {
        modal.toast({ message: "数据为空，无法修改" });
        return;
      } else storage.getItem("objbill", function (event) {
        var order1 = "";
        var order2 = "";
        if (_this2.checklist.length > 1) {
          order1 = _this2.checklist[0].bill_id;
          order2 = _this2.checklist[1].bill_id;
        } else {
          order1 = _this2.checklist[0].bill_id;
          order2 = "anull";
        }
        ...
        stream.fetch({
          method: "POST",
          url: app_server_url + "APPDriverList/Driver_BillStatus_Save?DriverID=" + JSON.parse(event.data).DriverID + "&bill_id=" + JSON.parse(event.data).bill_id + "&status=" + JSON.parse(event.data).bill_status + "&TYPE=03&bill_id_sub=" + order1 + "&DetailID=" + order2,
          ...
          if (ret.ok === false) {
            ...
            //  modal.toast({ message: "bjorder4"});
            if (_this2.checklist.length > 1) {
              _this2.checklist.forEach(function (element) {
                if (_this2.checklist.length > 1) if (element.StatusType == "02") {
                  valuebool = true;
                }
              });
              if (!valuebool) {
              ...
              myurl = app_server_url + "APPDriverList/Driver_BillStatus_Save?DriverID=" + _this2.DriverID + "&bill_id=" + _this2.checklist[0].bill_id + "&status=" + _this2.checklist[0].bill_status + "&TYPE=01&bill_id_sub=" + _this2.checklist[1].bill_id;
            } else {
              myurl = app_server_url + "APPDriverList/Driver_BillStatus_Save?DriverID=" + _this2.DriverID + "&bill_id=" + _this2.checklist[0].bill_id + "&status=" + _this2.checklist[0].bill_status + "&TYPE=05";
            }
            stream.fetch({
              method: "POST",
              url: myurl,
    ...
    posthttp: function posthttp() {
      ...
      storage.getItem("driverIndex", function (event) {
        // modal.toast({ message: "post="+event.data });
        _this.DriverID = JSON.parse(event.data).DriverID;
        // modal.toast({ message: "post="+ _this.DriverID });
        stream.fetch({
          method: "POST",
          ...
          if (ret.ok === false) {
            ...
            //   modal.toast({
            //     message: "-- userType --" + userType.factoryid_mc
            //   });
            datalist.forEach(function (element) {
              element.pkData = "请求交货日期:" + element.frmPkupDtt;
            ...
            //   modal.toast({
            //     message: "-- assignlist --" + _this.assignlist.length
            //   });
          }
        });
      });
