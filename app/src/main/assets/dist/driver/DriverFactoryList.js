...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/driver/DriverFactoryList.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-0939af4a"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  name: "DriverFactoryList",
  components: { WxcMinibar: _weexUi.WxcMinibar, WxcLoading: _weexUi.WxcLoading, WxcPartLoading: _weexUi.WxcPartLoading },
  data: function data() {
    return {
      lodinisShow: false,
      DriverID: "",
  ...
  methods: {
    ...
    nextKs: function nextKs(item) {
      this.objbill.DriverID = this.DriverID;
      this.objbill.factoryid = item.Code;
      storage.setItem("objbill", JSON.stringify(this.objbill));
      var num = this.$getConfig().bundleUrl.indexOf("DriverFactoryList.js");
    ...
    nextSpxx: function nextSpxx(item) {
      this.objbill.DriverID = this.DriverID;
      this.objbill.factoryid = item.Code;
      storage.setItem("objbill", JSON.stringify(this.objbill));
      var num = this.$getConfig().bundleUrl.indexOf("DriverFactoryList.js");
      var bundleUrl = this.$getConfig().bundleUrl.substr(0, num);
      // storage.setItem('driver_list', JSON.stringify(this.lists));
      //DriverFactoryList DriverScanning
      navigator.push({
        url: bundleUrl + "DriverScanning.js",
        animated: "true"
      }, function (event) {});
    },
    messageDetail: function messageDetail(item) {
      var _this = this;

      stream.fetch({
        method: "POST",
        url: app_server_url + "APPDriverList/Driver_Information_Save?DriverID=" + item.DriverID + "&ID=" + item.ID,
        ...
        if (ret.ok === false) {
          ...
          if (parseInt(ret.data.Data.BillStatusReturn[0].ai_value) == 1) {
            storage.setItem("MessageDetail", JSON.stringify(item), function (event) {});
            var num = _this.$getConfig().bundleUrl.indexOf("FMMessage.js");
            var bundleUrl = _this.$getConfig().bundleUrl.substr(0, num);

            navigator.push({
              url: bundleUrl + "FMMessageDetail.js",
              animated: "true"
            }, function (event) {});
            //  navigator.pop({}, e => {});
          } else {
  ...
  created: function created() {
    ...
    storage.getItem("sUserID", function (event) {
      ...
      // modal.toast({ message: "messageID:" + JSON.parse(event.data).DriverID });
      _this2.lodinisShow = true;
      stream.fetch({
        method: "POST",
        url: app_server_url + "APPDriverList/GetDriverBaseList?DriverID=" + event.data + "&TYPE=04",
        // url: `${app_server_url}APPDriverList/Driver_Security_Select?DriverID=${this.driverID}&shipid=${this.FactoryID}`,
        type: "json"
      }, function (ret) {
        //  modal.toast({ message: "bjorder2"});
        ...
        if (ret.ok === false) {
          ...
          if (ret.data.Data.DriverBaseList.length > 0) _this2.list = ret.data.Data.DriverBaseList;
        }
      });
    });
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    return _c('div', {
      ...
      staticClass: ["list-top-text", "iconfont"]
    }, [_vm._v(" 视频学习")])]), _c('text', {
      staticClass: ["cName"],
      on: {
        "click": function($event) {
