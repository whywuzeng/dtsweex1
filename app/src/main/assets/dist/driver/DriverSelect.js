...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/driver/DriverSelect.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-7491bfb9"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  name: "DriverSelect",
  components: { WxcMinibar: _weexUi.WxcMinibar, WxcLoading: _weexUi.WxcLoading },
  ...
  methods: {
    onchange: function onchange(event) {
      ...
      // modal.toast({ message: value });
      if (value.length >= 2) {
        _this.contentRefresh(value);
      } else {
        modal.toast({ message: "查询条件不得少于2个字" });
      }
    },
    clickCommitBtnClick: function clickCommitBtnClick() {
      var newList = [];
      var value = this.inputBid;
      var _this = this;

      // modal.toast({ message: value });
      if (value.length > 0) this.lists.forEach(function (item) {
        if (item.Name.indexOf(value) >= 0) {
          // modal.toast({ message: item.Name });
          newList.push(item);
          _this.lists = newList;
        }
      });else {
        _this.lists = _this.maxList;
        modal.toast({ message: "请查询" });
      }

      // this.contentRefresh();
    },
    selectItem: function selectItem(item) {
      item.type = this.isType;
      item.DriverID = this.UserId;
      storage.setItem("bottleData", JSON.stringify(item), function (event) {});
    ...
    //
    contentRefresh: function contentRefresh(value) {
      var _this2 = this;

      this.isShowLoadding = true;

      //   let sTruckText = this.inputBid == "" ? "anull" : this.inputBid;
      //   sTruckText = encodeURIComponent(sTruckText);
      var murl = '';
      if (this.isType == "Shipid") {
        murl = app_server_url + "APPDriverList/GetDriverBaseList?DriverID=" + this.UserId + "&Code=" + encodeURIComponent(value) + "&TYPE=21";
      } else {
        murl = app_server_url + "APPDriverList/GetDriverBaseList?DriverID=" + this.UserId + "&Code=" + encodeURIComponent(value) + "&TYPE=22";
      }
      ...
      stream.fetch({
        ...
        if (ret.ok === false) {
          ...
          // this.Err="ID:"+this.UserId+"isType:"+this.isType+"DriverBaseList:"+JSON.stringify(ret.data.Data);
          _this2.lists = [];

          if (ret.data.Data.DriverBaseList.length > 0) ret.data.Data.DriverBaseList.forEach(function (element) {
            //   this.Err="isType:"+isType+" / " +element.ListA[0].Name ;

            _this2.lists.push(element);
            // this.maxList.push(element);
          });
        }
        // this.Err=JSON.stringify(this.lists)
      });
    }
  },
  ...
  created: function created() {
    ...
    storage.getItem("isService", function (event) {
      if (event.result == "success") {
        //    modal.toast({ message: "isService:"+JSON.parse(event.data).DriverID});
        _this.isType = JSON.parse(event.data).type;
        _this.UserId = JSON.parse(event.data).DriverID;

        // _this.contentRefresh(JSON.parse(event.data).DriverID);
      }
    });
  },
