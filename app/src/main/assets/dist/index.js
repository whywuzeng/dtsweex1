...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/index.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-262cecc0"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
var app_server_url = 'https://tt.ab-inbev.cn/TrackApp/';
var app_server_enviroment = "pro_";
...
storage.getItem('app_server_enviroment', function (event) {
  if (event.data != null && event.data != undefined && event.data != "undefined") {
    if (app_server_enviroment == "dev") {
      app_server_enviroment = "dev_";
    } else if (app_server_enviroment == "pro") {
      app_server_enviroment = "pro_";
    } else app_server_enviroment = event.data;
  }
});

exports.default = {
  name: "Login",
  components: { WxcButton: _weexUi.WxcButton, WxcLoading: _weexUi.WxcLoading, WxcDialog: _weexUi.WxcDialog },
  data: function data() {
    return {
      bundleUrl: undefined,
      show: false,
      gcshow: false, // usertype2
      gcList: [], // usertype2
      title: '', //测试或正试
      locaUrl: "",
      state: true,
      isdisabled: false,
      username: "", //admin01425,20180210
      password: "",
      isShow: false,
      show_no_prompt: false,
      driverIndex: {
        DriverID: "",
        type: "",
        drivernext: false,
      ...
      updateurl: undefined,
      backImage_url: _app_constants.app_images_url + "login_bg.png",
      objbill: {},
      messageID: {
        DriverID: "",
        ID: ""
  ...
  created: function created() {
    ...
    var num = this.$getConfig().bundleUrl.indexOf("index.js");
    this.bundleUrl = this.$getConfig().bundleUrl.substr(0, num);
    storage.getItem(_app_constants.app_user_password, function (event) {
      if (event.data != null && event.data != undefined && event.data != "undefined") {
        _this2.password = event.data;
      }

      storage.getItem(_app_constants.app_username, function (event) {
        if (event.data != null && event.data != undefined && event.data != "undefined") {
          _this2.username = event.data;
  ...
  methods: {
    changeEnvironment: function changeEnvironment() {
      ...
      navigator.push({
        url: bundleUrl + "truckmanager/ChangeEnvironment.js",
        animated: "true"
      }, function (event) {});
    },
    viewappear: function viewappear() {
      ...
      storage.getItem('app_server_url', function (event) {
        if (event.data != null && event.data != undefined && event.data != "undefined") {
          app_server_url = event.data;
          if (app_server_url == 'http://139.219.133.22/DTSApi/') {
            _this3.title = '测试';
          } else {
            _this3.title = '';
          }
    ...
    ConfirmBtn: function ConfirmBtn() {
      ...
      navigator.push({
        url: bundleUrl + "driver/DriverTabs.js",
        animated: "true"
      }, function (event) {});
    },
    ...
    kwxcDialogConfirmBtnClicked: function kwxcDialogConfirmBtnClicked() {
      this.show = false;
      storage.setItem("home", 'N');
      var num = this.$getConfig().bundleUrl.indexOf("index.js");
      var bundleUrl = this.$getConfig().bundleUrl.substr(0, num);
      // storage.setItem('driver_list', JSON.stringify(this.lists));
      navigator.push({
        url: bundleUrl + "driver/DriverAnswer.js",
        animated: "true"
      }, function (event) {});
    },
    wxcButtonClicked: function wxcButtonClicked(e) {
      ...
      storage.getItem('app_server_url', function (event) {
        if (event.data === null || event.data === undefined || event.data === "undefined") {
          storage.setItem('app_server_url', app_server_url);
        }
      });
      ...
      storage.getItem('app_server_enviroment', function (event) {
        if (event.data === null || event.data === undefined || event.data === "undefined") {
          if (app_server_url == 'http://139.219.133.22/DTSApi/') {
            storage.setItem('app_server_enviroment', "dev_");
          } else {
            storage.setItem('app_server_enviroment', app_server_enviroment);
          }
        }
      });
      ...
      stream.fetch({
        method: "POST",
        url: app_server_url + "Account?userName=" + this.username + "&passWord=" + this.password + "&rememberMe=true",
        type: "json"
      }, function (ret) {
        ...
        if (ret.ok === false) {
          ...
          //登录成功后，告知极光推送服务，进行账号的绑定。

          (0, _DTSPlugins.SetJiguangPush)({
            userid: _this4.username,
            eviroment: app_server_enviroment
          }, function (err, ret) {});

          var userInfo = ret.data.Data.UserInfo[0];
          var userType = userInfo.UserType;

          //check update,is new version app awailable,open the update webpage.
          var currentVersion = weex.config.env.appVersion;
          var isIOS = weex.config.env.platform.toLowerCase() === "ios";
          var newestVersion = isIOS ? userInfo.iosvernumber : userInfo.vernumber;
          // modal.toast({ message: "currentVersion:"+currentVersion+"  newestVersion:"+newestVersion });
          if (currentVersion < newestVersion) {
            _this4.updateurl = isIOS ? userInfo.iosupdateurl : userInfo.updateurl;
            _this4.showUpdateDialog = true;
            return;
          }

          var isValid = userInfo.isValid;
          if (isValid == "0") {

            //无效用户只能登录司机
            if (userType === "1" || userType === "3") {
              modal.toast({ message: "登陆用户无效，请确认!" });
              return;
            }
          }

          var num = _this4.$getConfig().bundleUrl.indexOf("index.js");
          var bundleUrl = _this4.$getConfig().bundleUrl.substr(0, num);

          //厂区管理员："UserType": "3", 司机："UserType": "2", 派单员："UserType": "1"
          var detination = undefined;

          if (userType === "3") {
            detination = "factorymanager/FMTabs.js";
            // 保存 UserId、FactoryId
          } else if (userType === "2") {
            _this4.driverIndex.DriverID = userInfo.UserId;
            _this4.driverIndex.type = "04";
            _this4.driverIndex.Name = userInfo.Name;
            _this4.objbill.DriverID = userInfo.UserId;
            storage.setItem("objbill", JSON.stringify(_this4.objbill));
            storage.setItem("driverIndex", JSON.stringify(_this4.driverIndex), function (event) {});
            _this4.messageID.DriverID = userInfo.UserId;
            //    this.messageID.DriverID ="1000000024";
            _this4.messageID.ID = "1";
            storage.setItem("messageID", JSON.stringify(_this4.messageID), function (event) {});
            detination = "driver/DriverTabs.js";
          } else if (userType === "1") {
            _this4.messageID.DriverID = userInfo.UserId;
            _this4.messageID.ID = "10";
            storage.setItem("messageID", JSON.stringify(_this4.messageID), function (event) {});
            detination = "truckmanager/TMTabs.js";
          }
          //存储个人信息
          storage.setItem("sUserID", userInfo.UserId, function (event) {}); //FM
          storage.setItem(_app_constants.app_userid, userInfo.UserId, function (event) {}); //TM
          storage.setItem(_app_constants.app_bundle_url, bundleUrl, function (event) {});
          storage.setItem("sUserType", userInfo.UserType, function (event) {});
          storage.setItem("app_name", userInfo.Name, function (event) {});
          storage.setItem(_app_constants.app_user_password, _this4.password, function (event) {});
          storage.setItem(_app_constants.app_username, _this4.username, function (event) {});
          if (userType === "2") {
            if (userInfo.isClause == "N") {
              _this4.objbill.DriverID = userInfo.UserId;
              storage.setItem("objbill", JSON.stringify(_this4.objbill));
              _this4.show = true;
              return;
            ...
            stream.fetch({
              method: "POST",
              url: app_server_url + "APPDriverList/GetDriverBaseList?DriverID=" + userInfo.UserId + "&TYPE=19",
              type: "json"
            }, function (ret) {
              if (ret.ok === false) {
                modal.toast({ message: "网络访问失败" });
                ...
                if (ret.data.Data.DriverBaseList.length > 0) {
                  if (ret.data.Data.DriverBaseList[0].Code.length > 0 && ret.data.Data.DriverBaseList[0].Name.length > 0) {
                    _this4.gcshow = true;
                    _this4.gcList = ret.data.Data.DriverBaseList;
                  }
                } else {
                  _this4.gcshow = false;
                  navigator.push({
                    url: bundleUrl + detination,
                    animated: "true"
                  }, function (event) {});
                }
              }
    ...
    wxcDialogConfirmBtnClicked: function wxcDialogConfirmBtnClicked() {
      ...
      (0, _DTSPlugins.YtOpenurl)({ url: this.updateurl }, function (err, ret) {});
    }
  },
  mounted: function mounted() {
    var _this = this;

    var authChannel = new BroadcastChannel("EscMsg");
    ...
    authChannel.onmessage = function (event) {
      // modal.toast({ message: "退出消息" });
      if (event.data == 'bkMsg') {

        navigator.push({
          url: _this.bundleUrl + 'driver/DriverTabs.js',
          animated: "true"
        }, function (event) {});
        return;
      }
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    attrs: {
      "src": _vm.backImage_url
    }
  })]), _c('div', {
    staticClass: ["mainContent"]
  }, [_c('div', [_c('text', {
    staticClass: ["app-title-text1"],
    on: {
      "click": _vm.changeEnvironment
    }
  }, [_vm._v("Track & Trace")]), _c('text', {
    staticClass: ["app-title-text2"]
  }, [_vm._v("百威追鹰系统 " + _vm._s(_vm.title))])]), _c('wxc-loading', {
    attrs: {
      "show": _vm.isShow
    }
  }), _c('div', {
    ...
    attrs: {
      "title": "提示",
      "content": "有新版本APP可供更新",
      "show": _vm.showUpdateDialog,
      "single": "true",
      "isChecked": "false",
      "noPromptText": "false",
      "showNoPrompt": _vm.show_no_prompt
    },
    on: {
    ...
    staticClass: ["dialog-text"]
  }, [_vm._v("今天的安全措施是否准备妥当。")])])])]), _c('wxc-dialog', {
    attrs: {
      "title": "考试提醒",
      "show": _vm.gcshow,
      "single": false,
    ...
    on: {
      "wxcDialogCancelBtnClicked": _vm.CancelBtn,
      "wxcDialogConfirmBtnClicked": _vm.ConfirmBtn
    }
  }, [_c('list', {
    staticClass: ["gcScroller"],
    attrs: {
      "slot": "content"
    },
    slot: "content"
  }, _vm._l((_vm.gcList), function(item) {
    return _c('cell', {
      key: item,
      appendAsTree: true,
      attrs: {
      ...
      staticClass: ["gcList-code"]
    }, [_vm._v(_vm._s(item.Code) + _vm._s(item.Name))]), _c('text', {
      staticClass: ["gcList-name"]
    })])])
  }))])], 1)
},staticRenderFns: []}
module.exports.render._withStripped = true
