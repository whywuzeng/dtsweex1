...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/truckmanager/TMTruckAssign.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-5ad43336"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
var _DTSUtil = __webpack_require__(452);
...
exports.default = {
    ...
    data: function data() {
        return {
            ...
            sUserId: "",
            sTruckGcId: "",
            isMultiOrdersAllocation: "false",
            listOrders: [],
            CarrierCode: '',
            sQuantityTotalCount: 0
        };
    },

    methods: {
        //多张订单
        ordersButtonClick: function ordersButtonClick() {
            storage.getItem("app_bundle_url", function (event) {
                if (event.data === undefined || event.data === null) {
                    return;
                }
                navigator.push({
                    url: event.data + "truckmanager/TMMultileOrders.js",
                    animated: "true"
                }, function (event) {});
            });
        },
        ...
        //
        judgeAssign: function judgeAssign() {
            if (JSON.stringify(this.selectedTruck) != "{}" && JSON.stringify(this.selectedDriver) != "{}") {
                this.isAssign = true;
            }
        },
        viewappear: function viewappear() {
            ...
            storage.getItem("TMBackHome", function (event) {
            ...
            if (this.entercount > 0) {
                storage.getItem(_TMTruckConstants.tm_truck_selected, function (event) {
                    if (event.result == "success") {
                        _this.selectedTruck = JSON.parse(event.data);
                    }
                    _this.judgeAssign();
                });
                ...
                storage.getItem(_TMTruckConstants.tm_driver_selected, function (event) {
                    if (event.result == "success") {
                        _this.selectedDriver = JSON.parse(event.data);
                    }
                    _this.judgeAssign();
                });
                ...
                storage.getItem("tm_gctruck_selected", function (event) {
                    if (event.result == "success") {
                        _this.selectedgcTruck = JSON.parse(event.data);
                    }
                    _this.judgeAssign();
                });
            }
        ...
        viewdisappear: function viewdisappear() {
            this.entercount += 1;
            storage.removeItem("fromTMMultiOrdersAllocation", function (event) {});
        },


        //选择司机
        driverCellClicked: function driverCellClicked(e) {
            var num = this.$getConfig().bundleUrl.indexOf("TMTruckAssign.js");
            var bundleUrl = this.$getConfig().bundleUrl.substr(0, num);
            navigator.push({
                url: bundleUrl + "TMDriverSelect.js",
                animated: "true"
            }, function (event) {});
        },

        //选择车辆
        truckCellClicked: function truckCellClicked(e) {
            var num = this.$getConfig().bundleUrl.indexOf("TMTruckAssign.js");
            var bundleUrl = this.$getConfig().bundleUrl.substr(0, num);
            navigator.push({
                url: bundleUrl + "TMTruckSelect.js",
                animated: "true"
            }, function (event) {});
        },

        //选择挂车
        gctruckCellClicked: function gctruckCellClicked(e) {
            storage.getItem("app_bundle_url", function (event) {
                if (event.data === undefined || event.data === null) {
                    return;
                }
        ...
        //确认指派
        bottomButtonClicked: function bottomButtonClicked(e) {
            var _this2 = this;

            if (this.isAssign == false) {
                return;
            }
            this.isAssign = false;

            //拼接sSkuId，sQuantity参数
            if (this.assignBillList.length === 0) {
                this.sSkuId = "anull";
            } else {
                this.assignBillList.forEach(function (val, index, array) {
                    if (_this2.sSkuId == "") {
                        _this2.sSkuId = val.TMS_SKU_ID;
                    } else {
                        _this2.sSkuId = _this2.sSkuId + "," + val.TMS_SKU_ID;
                    }
                });
            }

            if (this.assignBillList.length === 0) {
                this.sQuantity = "anull";
            } else {

                this.assignBillList.forEach(function (val, index, array) {

                    _this2.sQuantityTotalCount += parseFloat(val.stepperNum);

                    if (_this2.sQuantity === "") {
                        _this2.sQuantity = val.stepperNum;
                    } else {
                        _this2.sQuantity = _this2.sQuantity + "," + val.stepperNum;
                    }
                });
                // modal.toast({message:'-----' + this.sQuantityTotalCount});
                if (this.sQuantityTotalCount == 0) {
                    modal.toast({ message: '待配送总数量不能为0!' });
                    return;
                }
            }

            if (JSON.stringify(this.selectedgcTruck) == "{}") {
                this.selectedgcTruck.id = "anull";
            }
            // modal.toast({message:`${app_server_url}TruckAssign/AssignSave?sAppointmentCode=${this.sAppointmentCode}&sBillType=${this.sBillType}&sTruckId=${this.selectedTruck.id}&sTruckGcId=${this.selectedgcTruck.id}&sDrvier=${this.selectedDriver.id}&sUserId=${this.sUserId}&sSkuId=${this.sSkuId}&sQuantity=${this.sQuantity}`});

            this.isShowLoadding = true;
            stream.fetch({
                method: "POST",
                url: app_server_url + "TruckAssign/AssignSave?sAppointmentCode=" + this.sAppointmentCode + "&sBillType=" + this.sBillType + "&sTruckId=" + this.selectedTruck.id + "&sTruckGcId=" + this.selectedgcTruck.id + "&sDrvier=" + this.selectedDriver.id + "&sUserId=" + this.sUserId + "&sSkuId=" + this.sSkuId + "&sQuantity=" + this.sQuantity,
                type: "json"
            }, function (ret) {
                _this2.sSkuId = "";
                _this2.sQuantity = "";
                ...
                if (ret.ok === false) {
                    ...
                    storage.getItem("app_bundle_url", function (event) {
                        ...
                        navigator.push({
                            url: event.data + "truckmanager/TMSubmitSuccess.js",
                            animated: "true"
                        }, function (event) {});
                    });
                }
        ...
        wxcStepperValueChanged: function wxcStepperValueChanged(index, item, e) {
            item.stepperNum = e.value;
        },
        ...
        contentRefresh: function contentRefresh() {
            ...
            stream.fetch({
                method: "GET",
                url: app_server_url + "/TruckAssign/GetindexinfoAppointmentCode?sAppointmentCode=" + this.sAppointmentCode,
                type: "json"
            }, function (ret) {
        ...
        //交货单数据
        GetAssDetailInfoAll: function GetAssDetailInfoAll() {
            var _this4 = this;

            // modal.toast({message:`${app_server_url}/TruckAssign/GetAssDetailInfoAll?sAppointmentCode=${
            //             this.sAppointmentCode
            //         }`});

            stream.fetch({
                method: "GET",
                url: app_server_url + "/TruckAssign/GetAssDetailInfoAll?sAppointmentCode=" + this.sAppointmentCode,
                type: "json"
            }, function (ret) {
                _this4.isShowLoadding = false;
                if (ret.ok === false) {
                    modal.toast({ message: "网络访问失败" });
                } else if (ret.data.Result !== "True") {
                    modal.toast({ message: ret.data.Info });
                    ...
                    ret.data.Data.result.ListC.forEach(function (val, index, array) {
                        val.checked = false;
                    });

                    _this4.listOrders = ret.data.Data.result;
                }
            });
        },
        ...
        //待派送数量
        getBillDetailAppointmentCode: function getBillDetailAppointmentCode() {
            var _this5 = this;

            stream.fetch({
                method: "GET",
                url: app_server_url + "/TruckAssign/GetBillDetailAppointmentCode?sAppointmentCode=" + this.sAppointmentCode,
                type: "json"
            }, function (ret) {
                ...
                if (ret.ok === false) {
                    ...
                    _this5.assignBillList = ret.data.Data.GetBillList;

                    _this5.assignBillList.forEach(function (val, index, array) {
                        val.stepperNum = val.ResidualQuantity;
                    });
                }
            });
        }
    ...
    created: function created() {
        ...
        storage.getItem(_app_constants.app_userid, function (event) {
            if (event.result == "success") {
                _this6.sUserId = event.data;
            }
        });

        storage.getItem("AppointmentCodes", function (event) {
            if (event.data === undefined || event.data === null) {
                return;
            }
            _this6.sAppointmentCode = event.data;
            _this6.GetAssDetailInfoAll();
            _this6.contentRefresh();
        });

        storage.getItem("TMOrdersAllocationCurrentServiceCode", function (event) {
            if (event.data === undefined || event.data === null) {
                return;
            }

            _this6.sBillType = event.data;
            // modal.toast({ message: " ---" + this.sBillType });

            if (_this6.sBillType === "D1") {
                _this6.checkdisabled = false;
            } else {
                _this6.checkdisabled = true;
            }
        });

        this.isMultiOrdersAllocation = "false";
        storage.getItem("isMultiOrdersAllocation", function (event) {
            if (event.data === undefined || event.data === null) {
                return;
            }
            if (event.data == "true") {
                _this6.checkdisabled = true;
                _this6.isMultiOrdersAllocation = "true";
            }
        });
        storage.setItem("isMultiOrdersAllocation", "false", function (event) {});
    },
...
* 截取url参数， 
* @param url : 一般传：weex.config.bundleUrl
* @param param : 要取的参数的key
* @return : 要取的参数的key的value
*/
var GetParamValue = exports.GetParamValue = function GetParamValue(url, param) {
...
export let  GetParamValue =  function(url, param) {
    ...
    {
      hash = hashes[i].split('=');
      if(param === hash[0]) { return  decodeURIComponent( hash[1] ); }//decodeURIComponent( hash[1] )
    }
    return undefined;
}*/
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    class: [_vm.isiphone ? 'scrollerTop134' : '', _vm.isipx ? 'scrollerTop160' : '']
  }, [_c('scroller', {
  ...
  }, [_vm._v("负载单号:" + _vm._s(_vm.listFirstItem.AppointmentCode))]), _c('text', {
    staticClass: ["time"]
  }, [_vm._v(_vm._s(_vm.listFirstItem.handeTime))]), _c('text', {
    staticClass: ["iconfont", "cell-shop-image"]
  }, [_vm._v("")]), _c('text', {
    staticClass: ["shop"]
  }, [_vm._v(_vm._s(_vm.listFirstItem.toShpgLocName))]), _c('text', {
    staticClass: ["iconfont", "cell-image"]
  }, [_vm._v("")]), _c('text', {
    staticClass: ["address"]
  }, [_vm._v(_vm._s(_vm.listFirstItem.destinationName))]), ((_vm.lists.length > 1 && _vm.isMultiOrdersAllocation == 'true')) ? _c('text', {
    staticClass: ["ordersNumber"],
    on: {
      "click": _vm.ordersButtonClick
    }
  }, [_vm._v(_vm._s(_vm.lists.length) + "张订单")]) : _vm._e(), _c('div', {
    staticClass: ["header-bottom"]
  }, [_c('div', {
    staticClass: ["header-bottom-flexbox"]
  }, [_c('text', {
    staticClass: ["header-style"]
  }, [_vm._v("业务类型:")]), _c('text', {
    staticClass: ["header-style", "h-text-color"]
  }, [_vm._v(_vm._s(_vm.listFirstItem.ServiceCode))])]), _c('div', {
    staticClass: ["header-bottom-flexbox"]
  }, [_c('text', {
    staticClass: ["header-code"]
  }, [_vm._v("商品编码:")]), _c('text', {
    staticClass: ["header-code", "h-text-color"]
  }, [_vm._v(_vm._s(_vm.listFirstItem.CommodityCode))])]), _c('div', {
    staticClass: ["header-bottom-flexbox"]
  }, [_c('text', {
    staticClass: ["header-line"]
  }, [_vm._v("线路:")]), _c('text', {
    staticClass: ["header-line", "h-text-color"]
  }, [_vm._v(_vm._s(_vm.listFirstItem.rateCode))])])])]), _c('div', {
    ...
    staticClass: ["iconfont", "themecolor"],
    attrs: {
      "slot": "label"
    },
    slot: "label"
  }, [_vm._v("")]), (_vm.selectedDriver.text) ? _c('text', {
    staticClass: ["cell-placeholder", "textcolor3"],
    attrs: {
      "slot": "value"
    },
    slot: "value"
    ...
    slot: "label"
  }, [_vm._v("")]), (_vm.selectedTruck.text) ? _c('text', {
    staticClass: ["cell-placeholder", "textcolor3"],
    attrs: {
      "slot": "value"
    },
    ...
    slot: "label"
  }, [_vm._v("")]), (_vm.selectedgcTruck.text) ? _c('text', {
    staticClass: ["cell-placeholder", "textcolor3"],
    attrs: {
      "slot": "value"
    },
    ...
    slot: "value"
  }, [_vm._v("请选择配送挂车")])])], 1), _c('div', {
    staticStyle: {
      height: "20px",
      backgroundColor: "#eeeeee"
    }
  }), _vm._l((_vm.listOrders.ListC), function(item) {
    return _c('div', {
      key: item,
      ...
      staticClass: ["iconfont", "themecolor", "pannel-tag"]
    }, [_vm._v("")]), _c('text', {
    ...
    }, [_vm._v("件数：" + _vm._s(item.totPce) + "箱")]), _c('text', {
      staticClass: ["panel-text", "pOne"]
    ...
    slot: "label"
  }, [_vm._v("")])]), _vm._l((_vm.assignBillList), function(item, index) {
    return _c('div', {
      key: index,
      staticClass: ["thirdflex-items"]
    }, [_c('div', {
      staticClass: ["steppercell"]
    }, [_c('text', {
      staticClass: ["cell-oddnumber"]
    }, [_vm._v("交货单号:" + _vm._s(item.ShipmentId))]), _c('text', {
      staticClass: ["steppercell-text"]
    }, [_vm._v("SKU:" + _vm._s(item.SKUID) + "/" + _vm._s(item.SKUName))]), _c('text', {
      staticClass: ["steppercell-weight-text"]
    }, [_vm._v(_vm._s(item.Weight) + "KG/" + _vm._s(item.Volum_HL) + "百升")]), _c('text', {
      staticClass: ["steppercell-num-text"]
    }, [_vm._v(_vm._s(item.ResidualQuantity) + "箱")]), _c('wxc-stepper', {
      staticClass: ["stepper"],
      attrs: {
        "defaultValue": item.stepperNum,
        "step": "1",
      ...
      on: {
        "wxcStepperValueChanged": function($event) {
          _vm.wxcStepperValueChanged(index, item, $event)
        }
      }
    })], 1)])
  })], 2)], 2)]), (_vm.isAssign) ? _c('text', {
    staticClass: ["bottom-button", "button-highted"],
    on: {
      "click": _vm.bottomButtonClicked
    }
  }, [_vm._v("确认指派")]) : _c('text', {
