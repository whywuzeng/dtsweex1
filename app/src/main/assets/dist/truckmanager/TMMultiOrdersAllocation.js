...
__vue_options__.__file = "/Users/mac/Documents/DTS/dtsweex1/src/truckmanager/TMMultiOrdersAllocation.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-1f3d1606"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
    components: { WxcMinibar: _weexUi.WxcMinibar, WxcButton: _weexUi.WxcButton, WxcLoading: _weexUi.WxcLoading },
    ...
    methods: {
        viewappear: function viewappear() {
            ...
            storage.getItem("TMOrdersAllocationCurrentServiceCode", function (event) {
                ...
                _this.serviceCode = event.data;

                // storage.getItem(app_userid, event => {
                //     if (event.result == "success") {
                //         this.UserId = event.data;

                //         storage.getItem("AppointmentAgentId", event => {
                //             if (event.result == "success") {
                //                 this.selectedAgentId = event.data;
                //                 this.contentRefresh();
                //                 }
                //         });
                ...
                storage.getItem("TMOrdersAllocationBillListURL", function (event) {
                    if (event.result == "success") {
                        _this.sURL = event.data;
                        _this.contentRefresh();
                    }
                });
            });
        },
        ...
        bottomrightButtonClick: function bottomrightButtonClick() {
            ...
            this.lists.forEach(function (val, index, array) {
                if (val.checked) {
                    _this2.selectArray.push(val);
                }
            });

            this.selectString = "";
            this.selectArray.forEach(function (val, index, array) {
                if (val.checked) {
                    if (_this2.selectString.length == 0) {
                        _this2.selectString = val.AppointmentCode;
                    } else {
                        _this2.selectString = _this2.selectString + "," + val.AppointmentCode;
                    }
                }
            });

            storage.setItem("AppointmentCodes", this.selectString, function (event) {});
            storage.setItem("isMultiOrdersAllocation", "true", function (event) {});
        ...
        selection: function selection(item) {
            ...
            var len = this.lists.filter(function (val, index, array) {
                return val.checked;
            });

            this.selectedCount = len.length;
        ...
        contentRefresh: function contentRefresh() {
            ...
            stream.fetch({
                ...
                if (ret.ok === false) {
                    ...
                    ret.data.Data.GetBillListTest.forEach(function (val, index, array) {
                        val.checked = false;
                    });
                    _this3.lists = ret.data.Data.GetBillListTest;

                    //待
                    // storage.getItem("TMOrdersAllocationBillList", event => {
                    //     if (event.data === undefined || event.data === null) {
                    //         return;
                    //     }
                    //     ret.data.Data.GetBillList = JSON.parse(event.data); 

                    //     ret.data.Data.GetBillList.forEach(function(val, index, array) {
                    //             val.checked = false;
                    //       });

                    //     this.lists = ret.data.Data.GetBillList;
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    return _c('div', {
      ...
      staticClass: ["company"]
    }, [_vm._v(_vm._s(item.destinationName))])])])
  }))]), _c('div', {
    staticClass: ["bottom"]
  }, [(_vm.selectedCount) ? _c('div', {
    staticClass: ["bottom-leftview"]
  }, [_c('text', {
    staticClass: ["bottom-leftButton-selecttip"]
  }, [_vm._v("当前已选择:")]), _c('text', {
    staticClass: ["bottom-leftButton-select"]
  }, [_vm._v(_vm._s(_vm.selectedCount) + "张订单")])]) : _c('div', {
    staticClass: ["bottom-leftview"]
  }, [_c('text', {
    staticClass: ["bottom-leftButton"]
  }, [_vm._v("请选择批量派送的订单")])]), (_vm.btnState == 0) ? _c('text', {
    staticClass: ["bottom-rightButton"]
  }, [_vm._v("确认分配")]) : (_vm.btnState == 1) ? _c('text', {
    staticClass: ["bottom-rightButton", "redbg"],
    on: {
      "click": _vm.bottomrightButtonClick
    }
  }, [_vm._v("确认分配")]) : _vm._e()])], 1)
},staticRenderFns: []}
module.exports.render._withStripped = true
